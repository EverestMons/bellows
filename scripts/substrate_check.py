...
def substrate_status(plan_path):
    ...
    # may carry `fold_baseline:` — the baseline's path, resolved through the ONE resolver
    # (cycle_check.resolve_fold_baseline) — and walk 0 records it when it arms the
    # baseline. Beside-the-plan remains the fallback for a same-repo cycle.
    manifest = cycle_check.parse_manifest_stanza(text) or {}
    baseline, declared = cycle_check.resolve_fold_baseline(plan_path, manifest)
    if not baseline:
        if declared:
