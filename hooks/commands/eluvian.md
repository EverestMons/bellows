...
the mini — use whichever `<bellows>/status.py` exists. `$ELUVIAN_WRAP_MEMORY` is
the memory repo (a git repo on the shop machine; a plain directory on the mini).

1. **Pull latest code (multi-machine law: every machine runs the newest committed state).** Enumerate the repos DYNAMICALLY — never a hand list: `$ELUVIAN_WRAP_ROOT`, every direct child directory of it with a `.git` and an `origin` remote, the `<bellows>` checkout when it lives outside the root, and `$ELUVIAN_WRAP_MEMORY` when it is a git repo (on the mini it is not — skip it and say so). For each: `GIT_TERMINAL_PROMPT=0 git fetch origin` (a fetch failure is a LOUD warning, not a stop); report ahead/behind vs upstream; if BEHIND and not diverged → `GIT_TERMINAL_PROMPT=0 git pull --ff-only`; if DIVERGED, or the ff-only pull refuses, or the tree blocks it → report loudly and TOUCH NOTHING — never merge, rebase, stash, or reset on the command's behalf. ⚠️ If the bellows repo received new commits AND a daemon is running here, compare the RUNNING daemon's sha (from `python3 <bellows>/status.py`) to the new HEAD and, when they differ, report **"bellows daemon restart needed — running old code"**: a live daemon keeps executing the code it loaded at start. (On the mini, STOPPED is the correct daemon state — no restart warning applies.)

2. **Read the doctrine.** Load `$ELUVIAN_WRAP_ROOT/ELUVIAN_PATH.md` in full. This is the governing process document.
...
4. **Recite AND assert the system wiring** (the map goes in the report so every machine's session starts from the same picture; each assert is a real check, not a recollection):
   ...
   Mechanical asserts (report each): `$ELUVIAN_WRAP_ROOT/GLOSSARY.md` exists; `<bellows>/lifecycle.db` exists; the lessons-forge DB exists on its HOME machine and only there — the Mac mini, `<lessons-forge checkout>/lessons-forge.db` (checkout resolved like the align hook: `ELUVIAN_WRAP_LESSONS_FORGE`, `$ELUVIAN_WRAP_ROOT/lessons-forge`, `~/Developer/lessons-forge`, `~/Developer/forge_lessons`); on every other machine its ABSENCE is the correct state (one live copy since 2026-09-01), and a 0-byte file at that path is a decoy to delete.
