...
4. **Recite AND assert the system wiring** (the map goes in the report so every machine's session starts from the same picture; each assert is a real check, not a recollection):
   ...
   Mechanical asserts (report each): `$ELUVIAN_WRAP_ROOT/GLOSSARY.md` exists; `<bellows>/lifecycle.db` exists; `$ELUVIAN_WRAP_ROOT/lessons-forge/lessons-forge.db` exists (where the checkout exists).
