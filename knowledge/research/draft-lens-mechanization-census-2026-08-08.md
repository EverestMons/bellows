...
| **R** | record-class — already covered by a SHIPPED plan_lint check or by the HELD rows-25/27/28 batch ((m), panel-content, negation-gap). **Name which check per finding.** ⚠️ The shipped/held boundary here is AUTHORING-TIME ((f)–(l) shipped as of 2026-08-08); **the run re-derives the shipped set from the live `plan_lint.py` at the worktree's HEAD — the held batch may land in-window, and Q4's executed slice keys on the LIVE set, never this cell.** | §3/§4 |
...
**Q4 — The re-finding decomposition (prices the post-fold battery).** For every Q1 finding, classify: **novel** vs **introduced-by-a-prior-fold** (evidence: the commit narrative — messages like "the a1's Q0 class", "fold-residue" — plus diff-overlap with prior phase hunks; **state the method and mark low-confidence rows**). Then: of the prior-fold-introduced set, what fraction classified R or M1–M8/O. ⚠️ **That fraction is a CEILING for a fold-time battery, not a catch prediction — most of those checks do not exist yet and a classification is not a fire. Report it as the ceiling, labeled as such.** **Except the slice that CAN be executed: for every R-bucket finding whose named check is in the LIVE shipped set — re-derived at run from the worktree-HEAD `plan_lint.py` per the taxonomy R-cell rule, NOT from any enumeration frozen in this plan — run the current `plan_lint` against that finding's pre-fold state and report fires/doesn't per finding; that slice moves from ceiling to MEASURED, and the still-unshipped R slice stays ceiling-marked.** ⚠️ **Population match before comparison: doctrine's "roughly a third" and 11/12/12/12/12 describe COLD-PANEL rounds, not whole cycles. Report the re-finding rate TWICE — whole-cycle, and restricted to panel-seat rounds — and compare ONLY the panel-round slice against doctrine's figure, with the whole-cycle rate reported as this census's own new number.** State the divergence, if any, plainly.

**Q5 — Extension census (explicitly optional, never silent).** Repeat Q1+Q2 (classification only, no prototypes) for **315 (bellows, 9 commits — the one T1-diagnostic cycle, added for tier diversity: the required corpus is all-T2 and a build decision fitted only to T2 cycles would over-fit)**, **306 (bellows)**, and **309 (shop root, 31 commits)**. ⚠️ **306 predates the close-commit convention — verified at authoring: no `close(306)` commit exists in bellows, so the Q1 recipe's step 1 fails there; fall back to enumerating deleted draft files (`git log --diff-filter=D --name-only`, path-filtered under `knowledge/`) to find its draft path, or report the cycle NOT ENUMERABLE with the probe evidence — never force the recipe.** ⚠️ **If not reached, the deposit says "NOT REACHED" for Q5 with the reason — an explicit cut, never a silent one; partial order is 315 → 306 → 309, cut from the tail.**
...
- The three cycles span three repos, addressed ABSOLUTELY (a bellows worktree's relative paths resolve against the worktree): **311 → `/Users/marklehn/Developer/GitHub/lessons-forge` · 317 → `/Users/marklehn/Developer/GitHub/bellows` · 320 → `/Users/marklehn/Developer/GitHub` (the shop root; its log also carries OTHER cycles' `[draft]` commits — 309, codify-d — which is why the population filter is the draft PATH, never the message prefix).** Extension roots (Q5): 315 → bellows, 306 → bellows, 309 → the shop root. **Re-derive each draft path per the Q1 recipe; never assume the authoring-time examples still hold.**
...
**Confirming:** c1 (four lenses, rotated to the under-examined regions: header, Why block, taxonomy, extension scope) — 4 folded, NOT dry (320's elided draft path recovered in full; R-bucket shipped/held boundary made run-time-derived vs the in-window held batch; Q5 gains 315 for tier diversity + explicit partial order; 306's missing close(306) measured live → recipe fallback, never forced). c2 (four lenses, rotated to deposit contract / Receipt / header / Expected-lint) — 1 folded (contract item 4 synced to the a1 dual-slice form — the instruction-vs-record class, specimen 4), plus the §5 conformance pass run as mandated pre-close: lint 3 PASS · 0 WARN · exit 0, Expected-lint line finalized EARNED. c3 (four lenses, read from the bytes not the working model) — **DRY AT ZERO, per-lens: Weak spots dry · Destruction dry · Vulnerabilities dry · Integration-record dry** (two candidates weighed and rejected with reasons: the line-25 narrative phrasing is qualified explicitly in the adjacent bullet; the ID note's re-token clause is vacuous-but-harmless since no in-body id sites remain).

**Closing:** the last event is a lens pass — c3, a full four-lens confirming read, returned dry at zero. Cycle: w1 (26, per-lens) → a1 (5, apart) → c1 (4) → aC (2, apart, re-read dry) → c2 (1 + §5 pass) → c3 (dry at zero). 38 folds, all landed, per-phase committed. Lint EARNED at 0 WARN, re-verified after this record write. Not yet deposited — deposit once, on CEO direction, with the id re-read from `id_sequence` at deposit per the header note.
