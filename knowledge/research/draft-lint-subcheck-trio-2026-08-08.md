...
>
> **Run targeted tests only:** `python3 -m pytest tests/ -k "plan_lint or lint" --tb=short -q 2>&1 | cat`. ⚠️ **Do NOT run the full suite — that is Step 2's job.** Then run `plan_lint` live against one real compliant plan and one deliberately-tripping fixture — **the pasted RAW output must show ALL FOUR labels firing — `(n)`, `(o1)`, `(o2)`, `(p)` — plus `echo $?` = 0 (cold seat 3: "(o)" was satisfiable by o2 alone with o1 dead). For o1 to fire, the fixture must live under a `/knowledge/`-rooted tree OR carry an absolute missing `/Users/…` candidate — a bare-tmp fixture's relative candidates skip by design. Paste the FIXTURE TEXT itself in the dev log verbatim: Step 2's live positive control re-materializes it from there.**
>
> **Scope:**
...
**Cold seats (running record, author-verified per seat):** seat-1 Weak-spots-cold 7 findings (2 HIGH: the v0 exclusion silenced the measured TP class → (o) split into o1+o2; the QA HALT was tautological-or-guaranteed → independent re-derivation) · seat-2 Destruction-cold 9 findings (2 HIGH: A0 restore re-hardened to hunk attribution; o1 over-exclusion made visible via INFO accounting + live control; 306 sweep-diff proof restored) · seat-3 Vulnerabilities-cold 7 findings (2 HIGH: skip-vs-INFO contradiction pinned; the tripping fixture never required to trip o1 → all-four-labels + fixture-placement + paste-the-fixture; plus UNCAP tested, megaspan single-line bound, fresh-dir pre-lint + post-edit blob pin, INFO triple defined, single-quote fixture).
...
**Cold panel (T2):** convening under a CEO autonomous-panel grant (2026-08-08, this cycle — the second such grant; the session-27 grant was bounded to 320's panel and did not carry): all five seats run on one directive, sequentially, author-verified, per-seat committed; waiver recorded here per the 320 precedent. Seats = the five lenses cold, aimed per §2.6 at the deletion/narrowing premises (the dot-widening, (o)'s filter set, the stated misses) and the clone-diff against 303/306. (This draft's own v0 used a non-canonical `Panel status:` label and the shipped line-anchored check caught it — the row-27 class, live.)
