...
**Walks:** 1, plus ACID pass a1 (its own turn per the standing phase direction).
...
- ACID:                a1 5 folded (5.2 Q3×Q2 scope contradiction — the cross-check w1 wrote could not run on pre-07-02 rows, re-based whole-table; headline rate re-based to one matched numerator/denominator pair at Q2's exact scope; plan-level time-axis limitation stated in Q2/Q4; 5.3 live-DB bookend totals mandated; 5.4 verdict-files-read named in Q3's deposit).
**Conflicts:** none recorded — a1's two Consistency findings were contradictions BETWEEN question requirements, resolved in one move each, not a lens-constraint violated by a fold.
**Closing:** pending — a1's last event is a fold; §2 requires the last event before deposit to be a lens pass, so a confirming pass is owed. Not deposited.
