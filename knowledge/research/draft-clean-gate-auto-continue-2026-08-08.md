...
Diagnostic-315 measured the trade the CEO has now taken: on clean-gate pauses the Planner's read catches something **3.08%** of the time (10/325 post-07-02, Wilson CI [1.7%, 5.6%]), while the mechanizable class (`header_pause` + `qa_checkpoint`) constitutes **83.3%** of all pauses — 317 clean-gate continues in five weeks overall, **314 of them in the mechanizable class** (~1.53 per plan). This plan mechanizes a deliberately narrowed slice of that class:
...
- **OUT: any gate failure** — the pause cascade checks `gate_result["passed"]` before the header, so a failed gate pauses regardless of mode. 315 Q4's population (13.5% of pauses; the `gate_failure` share alone is 12.4%) is structurally unaffected.
...
**Notification decision (explicit, CEO-overridable at this plan's gate):** the mechanical advance fires **no notification** — matching the already-existing silent mechanical path it widens (315 Q6 enumerated the three options; a per-advance push would fire on every multi-step plan). The `gate_auto` row is the audit trail; the dashboard's awaiting-verdict surface is unaffected in normal operation because the paired calls set the outcome immediately. (Both lifecycle writes are fail-soft — a swallowed outcome write would strand a NULL row visible in the awaiting surface, the 310 shape, remedied by the same manual close; the mirror direction — a swallowed request INSERT followed by a zero-row outcome UPDATE — leaves no row at all, an unrecorded advance with no visible signal, and the post-activation canary is the check that catches that direction.) **Ledger write — explicit out-of-scope:** 313's auto-close records to BOTH channels because a pre-existing `log_to_ledger("auto-close")` call already sat on that path; the mechanical-advance path has no ledger write and this plan adds none — clean-gate advances are recorded in lifecycle.db only, and `ledger.jsonl` parity for this transition class is declared out of scope here, not omitted silently.
...
>
> **Task A0 — pre-edit cleanliness.** `git -C /Users/marklehn/Developer/GitHub/bellows status --porcelain -- bellows.py verdicts/README.md scripts/plan_lint.py validators.py tests/test_gate_transaction_mechanization.py tests/test_bellows.py tests/test_plan_lint.py tests/test_validators.py` must be empty. If DIRTY, enumerate the hunks, attribute each to this plan; any unattributable hunk → HALT, do not restore.
>
...
> - **Site 2 (mechanical-advance provenance):** inside the non-terminal while-loop, immediately after the pause conditional's closing `return`-block and BEFORE the line anchored `default_next_prompt = f"Read the plan at {shadow_prompt_path}. Execute Step {current_step + 1}.`, add (guard with `if plan_id:` — a deliberate belt-and-braces choice, redundant with lifecycle's internal None-check; NOTE the loop's own `record_verdict_request` at the pause site is unguarded and 313's Site 2 shipped unguarded, so this is a style decision, not precedent-following):
>   `if plan_id:`
...
>   with a one-line comment stating this records the mechanical clean-gate continue so the transition is auditable (315's evidence; clones 313's auto-close pattern). The request+outcome pairing is REQUIRED — the outcome call without the request call is a silent no-op. ⚠️ Precision on the semantics: `record_verdict_outcome` updates EVERY row matching plan+step with a NULL outcome (`lifecycle.py`, anchor `"""Update the most recent pending verdict row` — the docstring's "most recent" overstates the SQL, which has no ORDER BY or LIMIT). A pre-existing stranded NULL row for the same plan+step would be stamped by the same UPDATE — unreachable through current paths for the RIGHT reason: the pending row is stamped at verdict CONSUMPTION before any re-dispatch (a continue advances to N+1; a precondition-failure continue retries step N, but only after its row is already stamped) — and pinned by Task D's two-row case rather than assumed away.
> - **Site 3 (`verdicts/README.md`):** in the pause-reason table (anchor: the row `| `auto_close_disabled` | Auto-close is disabled for the plan |`), add TWO rows: `auto_close` (mechanical terminal auto-close, recorded since plan 313 — a pre-existing documentation gap backfilled here, disclosed in the dev log) and `clean_gate_auto` (recorded for a mechanical clean-gate non-terminal advance — NOT a pause; the row exists so the transition is auditable). ⚠️ The README's "pauses plan execution under five conditions" sentence stays FIVE — the two new rows are NOT pauses; instead add a half-sentence distinguishing recorded transition codes (`auto_close`, `clean_gate_auto`) from pause reasons, so the table's row count and the pause count cannot read as contradicting. Also correct the STALE `header_pause` row (it currently reads `pause_for_verdict: true` — the field's real values are the four modes incl. the new one), disclosed in the dev log like the auto_close backfill. Add one line documenting the `qa_and_terminal` header mode where the README discusses pause behavior, noting that a terminal-step pause under this mode records `pause_reason_code=header_pause` (indistinguishable in the table from an `always` pause).
> - **Site 4 (the other two copies of the recognized-value list):** the mode's enum lives in THREE places, and shipping one is the defect Site 1 already names, one directory over. (a) `scripts/plan_lint.py`, anchor `RECOGNIZED_PAUSE_TOKENS = {"always", "after_step_1", "after_qa_step"}` — add `"qa_and_terminal"`; without it every future opted-in plan, INCLUDING the mandatory post-activation canary, draws a hard lint FAIL at the pre-deposit check. Add a case to `tests/test_plan_lint.py`, named so it contains `qa_and_terminal` (that name is its Task E selection term), asserting a header with the new mode passes check (a). (b) `validators.py`, anchor `VALID_PAUSE_FOR_VERDICT_VALUES = {"always", "after_step_1", "after_qa_step", "after_each_step", ""}` — add `"qa_and_terminal"`; without it every opted-in plan draws a claim-time WARN whose text is false twice over (the value would be recognized in code, and the defensive default does not apply to a set value). Extend `tests/test_validators.py` with the matching case, same naming rule. Disclose in the dev log: validators' `after_each_step` is a GHOST value `header_says_pause` never implemented — record it, do not fix it here.
...
>
> **Task C — mode tests.** Add a test class named `TestHeaderSaysPauseModes` to `tests/test_gate_transaction_mechanization.py` exercising the REAL `header_says_pause` — import from `bellows` exactly as `tests/test_bellows.py` already does (proven side-effect-free). `tests/test_bellows.py` itself is read and run but expected UNCHANGED — it stays in Scope as authorization and is deliberately NOT a declared deposit. Cases: `qa_and_terminal` returns False for a non-terminal non-QA step; True for a QA step; True for the terminal step — asserted unconditionally, which IS the auto-close precedence pin (the function does not read `auto_close`, so terminal-True holds regardless of that header); True for every step when `total_steps` is 0 (unparsed-header degenerate — the safe direction); True at step 1 of a single-step plan (mode misuse on a diagnostic is harmless); the three existing modes return unchanged values on the same inputs; an unrecognized value still returns False.
>
> **Task D — provenance test.** Add a test class named `TestCleanGateAutoProvenance` to `tests/test_gate_transaction_mechanization.py` at the lifecycle layer (per-test DB, the existing conftest isolation): the `clean_gate_auto` request+outcome pair produces a queryable row with `outcome='continue'`, `decided_by='gate_auto'`, `pause_reason_code='clean_gate_auto'` — AND that row does NOT match the dashboard's awaiting filter (`SELECT ... WHERE outcome IS NULL` returns nothing for it). Include a TWO-ROW case: insert two pending rows for one plan+step, run the outcome call once, assert BOTH rows are stamped — the characterization that pins the UPDATE's actual all-pending-rows semantics (it flips the day the UPDATE is scoped, which is the signal to update the record). **A `run_plan`-level integration assertion is MANDATORY — not deferrable:** the lifecycle-layer tests never touch `bellows.py`, and the presence grep cannot see PLACEMENT — a pair inserted on the wrong side of the pause conditional would stamp genuine awaiting-verdict rows as `gate_auto` continues and still pass every other mandated check. The practicality precedent already ships in `tests/test_bellows.py` (anchor: `patch("bellows.header_says_pause"` — the side_effect spy pattern driving `run_plan`): drive a multi-step plan whose non-terminal step advances mechanically and assert the `clean_gate_auto` row lands for that step, AND drive a paused run and assert NO such row is written — the placement discrimination is the pair of those two assertions.
>
> **Task E — run targeted tests ONLY** (never the full suite in DEV): `python3 -m pytest tests/test_gate_transaction_mechanization.py tests/test_bellows.py tests/test_plan_lint.py tests/test_validators.py -k "verdict or decided or auto_close or transaction or header_says_pause or clean_gate or qa_and_terminal" --tb=short -q 2>&1 | cat`. Selection mechanics (Planner-verified by collect-only probe): the mechanization file's tests are all selected via the MODULE-name term `transaction`; the new plan_lint/validators cases are selected via the `qa_and_terminal` term their names are REQUIRED to contain (Site 4's naming rule — the class names elsewhere are organization, not the selection mechanism). Paste RAW output UNTRUNCATED. ⚠️ `$?` after the pipe is `cat`'s exit and CANNOT fail — report `echo "pytest_exit=${pipestatus[1]}"` (zsh) immediately after the run, or run pytest bare without the pipe; the RAW pytest summary line is the primary evidence either way, and it must show all selected tests passing.
>
> **Scope:**
> - `bellows.py`
> - `verdicts/README.md`
...
>
> **Task Q0 — re-pin state.** `git -C /Users/marklehn/Developer/GitHub/bellows log -1 --oneline -- bellows.py verdicts/README.md scripts/plan_lint.py validators.py tests/test_gate_transaction_mechanization.py tests/test_bellows.py tests/test_plan_lint.py tests/test_validators.py` — the most recent commit touching any must be Step 1's (a daemon `bellows: auto-stage declared deposits` commit tagged with this plan's id counts as Step 1's — do not HALT on it). A foreign commit → HALT and report.
>
...
> 3. **Grep-proof all five edit surfaces** (bare `grep -F`, never through a pipe; state what each prints on success and on failure): `grep -F "qa_and_terminal" bellows.py` prints ≥2 lines (mode branch + WARN literal) and exits 0; `grep -F "clean_gate_auto" bellows.py` prints ≥1 line and exits 0; `grep -F "clean_gate_auto" verdicts/README.md` prints ≥1 line and exits 0; `grep -F "qa_and_terminal" scripts/plan_lint.py` prints ≥1 line and exits 0; `grep -F "qa_and_terminal" validators.py` prints ≥1 line and exits 0.
...
>    Deposit both evidence files BEFORE running the block — it `sys.exit(1)`s if any is missing or empty. Include the block's literal stdout. The banner `Rule 20 — QA Self-Check Results` and the `PASSED — SELF-CHECK PASSED` line must appear BYTE-EXACT (em-dash U+2014). If it prints FAILED, HALT. The report carries **a verification row per numbered item with its raw evidence, in a table under a `##` heading containing `verification`** — the rule_22 (c)/(d) gates scan ONLY such tables; a prose-only report makes them vacuous. ⚠️ Keep every hedging keyword — especially the word "pending" — OUT of any table row carrying a status token: the canonical block CRITICAL-fails a positive-status row containing one, and Task D's two-row case is naturally described with exactly that word; quote its assertion in prose or a fenced block, never inside the verification table. Seat-3 disclosure: both evidence filenames already exist from 313's QA, so the block's existence check alone cannot prove freshness — the RAW summary lines from items 1–2 are the freshness evidence the Planner reads.
...
>
> (Both bullets above are blockquoted step text like every other instruction line; the agent's emitted receipt block carries them contiguous, no blank line between.)
>
> **Scope:**
...
- aC (ACID confirming, apart): dry at zero — 5.2 pair-sweep: A0/Q0/HALT/Scope/Deposits agree on one five-file surface, Site 3 internally consistent, Step 2's evidence-before-block ordering holds, Forward bullets match the deferral bullet; systemic: Site 2's insertion verified against the advance path's ordering (no teardown between continuous steps; completed step's row lands before the next step's start row), mode × recording coherent for all three header populations; 5.1/5.4 clean; 5.3 one disposition — the shared rolling full-suite evidence file is pre-existing convention, no new window. ⚠️ CORRECTED at seat 2 (attestation integrity — a correction, not a silent rewrite): the pair-sweep enumerated an INCOMPLETE surface. The recognized-value invariant spans THREE files (the bellows.py WARN literal, scripts/plan_lint.py's token set, validators.py's enum); the sweep as run covered only the first, and seat-2 findings 1–2 (lint FAIL for every opted-in plan incl. the canary; false claim-time WARN) landed exactly in the uncovered two. Folded as Site 4 with the full A0/Q0/HALT/Scope/Deposits/Task-E sweep.
**Cold panel (T2):** seat 1 run (fresh reader; all anchors byte-verified, all four deletion premises attacked): 8 findings, all author-verified, culminated (2 MEDIUM — the outcome-UPDATE "most recent pending" premise is false vs the actual unscoped SQL, Task B re-worded + Task D two-row characterization added; the ledger-half drop from the 313 clone was undisclosed, now declared out of scope explicitly; plus the phantom test_bellows.py deposit dropped, two number slips vs the 315 deposit corrected — 314 mechanizable / 13.5% Q4, the Forward bullets re-blockquoted, the invisible fail-soft direction acknowledged with the canary as its check, and Site 3 extended to fix the README's stale header_pause row + note the terminal-pause ambiguity). ⚠️ All four deletion premises SURVIVED cold falsification: QA exclusion (cascade order), terminal precedence (auto-close branch unreachable), silent-path claim (no notifier call on the advance path), consumer analysis (nil, with positive controls). Seat 2 (fresh reader; rotation aimed at first-step flow, resume path, conftest isolation — all three clean): 5 findings, all author-verified, culminated — 1 HIGH: the recognized-value list is a THREE-copy enum and the plan updated one; `scripts/plan_lint.py` would hard-FAIL every opted-in plan including the mandatory canary, `validators.py` would emit a doubly-false claim-time WARN — folded as Site 4 + full surface sweep (Scope 9 files, Deposits 8, A0/Q0/HALT extended, Task E gains the `qa_and_terminal` term with its naming rule); plus the aC record corrected under attestation integrity, the Site 2 guard re-labeled belt-and-braces (the cited precedent was false — :771 is unguarded, as was 313's Site 2), the stranded-NULL parenthetical re-based on the true mechanism (row stamped at consumption; precondition-failure retries step N), and validators' `after_each_step` ghost value disclosed to the dev log. Seat 3 (fresh reader; rotation aimed at the harness/QA execution surface — gates, parser, Rule 20 block, step extraction): 8 findings, all author-verified, 7 culminated — 1 HIGH: the central deliverable's PLACEMENT was proven only by presence-grep + lifecycle tests, and Task D pre-supplied a false deferral excuse while the run_plan spy harness already ships in test_bellows.py — the integration assertion (row lands on mechanical advance AND no row on a paused run) is now MANDATORY; plus the vacuous `$?`-after-pipe check replaced with pipestatus, the "pending"-keyword false-HALT trap defused in item 5, 313's dropped verification-table mandate restored (rule_22 c/d scan only such tables), the stale-evidence satisfiability disclosed with raw summary lines named as the freshness evidence, qa_steps re-formed to the primary-parser shape, and Q0's auto-stage false-HALT allowance added; finding 8 (gate-span absorption) verified benign, no action. Seats 4–5 owed.
**Conflicts:** none recorded — no fold violated a prior lens's constraint at any culmination.
**Closing:** pending — the warm cycle closed (c1 dry, aC dry at zero); the cold panel is in progress: seat 1 culminated (the fold above), seats 2–5 owed, each its own turn. Not deposited.
