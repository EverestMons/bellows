...
**Population context from Step 2 audit:** The exhaustive plan-header scan found 17 class (b) leaks out of 142 QA steps (12.0% overall leak rate; 65.2% in invoice-pulse alone, 2.1% in bellows). Six leak patterns identified:
...
4. Bare `## STEP N` with no role name -- 2 occurrences (standalone/superseded QA plans where the header lacked role annotation but the body assigned Invoice Security & Testing Analyst)
5. `Bellows Security & Testing` -- 1 occurrence (bellows variant, same "Security & Testing" pattern)
6. `Canary verification` with `**Agent:** QA` in body -- 1 occurrence (header uses descriptive label, not role name; gate checks only the header line)

The `qa_steps` field fixes all six patterns because it is role-name-independent and header-text-independent.

**Downstream interaction:** The `pause_for_verdict: after_qa_step` header value (already in use on invoice-pulse plans) is currently inert when the QA role name lacks "qa" -- the pause fires only if `is_qa_step` is True, which the keyword fallback never produces for these plans. Fixing `_gate_is_qa_step` via the `qa_steps` field also restores `after_qa_step` pause semantics for these plans.
...
No change needed to `_parse_plan_header()` itself (gates.py:79-145). The function already extracts arbitrary bold-Markdown fields via regex:
```python
...
Replace the current function (gates.py:543-549) with:
...
At gates.py:178, pass the already-parsed header:
```python
# Before:
...
**Interaction with Rule 25 mechanized-check routing.** The critical path where the leak causes harm is Rule 25's post-verdict-enrichment logic (PLANNER_TEMPLATE.md:685): when BOTH `rule_22_verification: PASS` AND `rule_20_self_check: PASS`, the Planner performs substance check (b) ONLY. A false `is_qa_step = False` produces vacuous PASS results for both gates, which Rule 25 treats as clean -- suppressing the Planner's manual Rule 20 verification. The `qa_steps` field fixes this by ensuring `is_qa_step` is True for all declared QA steps, which causes the gates to actually verify Rule 20 banner existence and Rule 22 QA-specific checks. When those checks produce genuine PASS results (not vacuous N/A), Rule 25's (b)-only routing is correct and safe.

## 4. Governance Edit

### Insertion point

The `qa_steps` field should be documented in three places in PLANNER_TEMPLATE.md:

**Place 1: Plan File Structure section** (currently line 349). The header example line should include the new field:
...
**Place 2: New paragraph after the "Execution map" paragraph** (currently line 398) and before the "Every agent prompt must include the domain glossary read" paragraph (currently line 400). This is the definitional paragraph:

**Proposed paragraph wording:**

> **`qa_steps` header field.** Every executable plan with one or more QA steps MUST declare a `qa_steps` field in the plan header listing the step numbers that are QA-gated, as a comma-separated list of integers (e.g., `qa_steps: 2` or `qa_steps: 2,4`). Bellows's `_gate_is_qa_step` reads this field to determine which steps trigger Rule 20 self-check verification, Rule 22 QA-specific checks (c)+(d), and `qa_checkpoint` auto-pause. Without the field, Bellows falls back to keyword detection on the `## STEP N` header line -- checking whether the header contains the substring "qa" (case-insensitive). This fallback misclassifies QA steps whose specialist role name does not contain "qa" (e.g., "Invoice Security & Testing Analyst", "Bellows Security & Testing") or whose header uses a descriptive label instead of a role name (e.g., "Canary verification" with `**Agent:** QA` in the body). The Step 2 population audit found 17 leaked QA steps across 142 total (12.0% overall; 65.2% in invoice-pulse) -- every instance of "Invoice Security & Testing Analyst" leaked, every use of a descriptive header label leaked. The `qa_steps` field is the authoritative source; the keyword fallback is a backward-compatibility path for legacy plans. Diagnostic plans and plans with no QA steps omit the field entirely -- omission means "no QA steps" when no keyword match is found.

This paragraph belongs at this position because it describes a header field (like the "Execution map" paragraph immediately above it) rather than a prompt-level instruction (like the "domain glossary read" paragraph below it). It is logically adjacent to the `pause_for_verdict` header field documentation (line 979) but belongs in the Plan File Structure section because it is a structural field the Planner must author, not a Bellows-mechanics section.

**Place 3: Gate 6 description in "The Eight Gates" table** (currently line 955). The current Gate 6 row reads:
...
| **YAML frontmatter plans** | Rare -- most plans use bold-Markdown headers. | `_parse_plan_header` handles YAML frontmatter as Strategy 1 (gates.py:91-100). A YAML field `qa_steps: [2, 4]` would parse as a list, not a string. The proposed code explicitly handles both cases: `isinstance(qa_steps_raw, list)` branch for YAML lists, `str().split(",")` for bold-Markdown strings. Both paths produce correct results. |
| **Body-level `**Agent:** QA` designation (new vector)** | Low frequency (1/142 in audit window) but structurally possible whenever the Planner uses a descriptive header label. | The `qa_steps` field eliminates this vector entirely because it is checked before and independently of header text. Plans authored under the updated PLANNER_TEMPLATE will always declare `qa_steps` for QA steps regardless of header label. Legacy plans with this pattern remain broken under keyword fallback (same as today). |
| **Stale running daemon during fix-plan execution** | Certain -- the fix-plan modifies `_gate_is_qa_step`, which the running daemon uses during the fix-plan's own QA step. | Per PLANNER_TEMPLATE.md:1031 (Restart Discipline), parser fix-plans trip their own bug. The Planner pre-documents the expected CEO override in the plan header. After daemon restart, a canary plan with `qa_steps: 1` and a non-"qa" header label confirms the new code loaded correctly. |
