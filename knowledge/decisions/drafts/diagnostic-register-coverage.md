...
**Blocker CLEARED.** Thread **141** (which superseded 140) shipped as `065c57a`: `cycle_check` now consults the walk register before returning `CONTINUE` on an empty Walks block, so the advance signal can no longer go silent on the exact failure that hid this cycle's own recording error for two walks. ⛔ **This plan is one of the two artifacts that fix fires on** — its register carries rows, so if its Walks block ever empties, the tool now says so.

⚠️ **Resume conditions, checked 2026-09-05:** `plan_lint` 0 FAIL · `cycle_check` CONTINUE (computed from `walk_data [1,2]`, not an empty record) · register CONFORMANT, 18 rows, 0 non-OK · substrate 4/4. Walk 3 opens on lens 1 over the whole artifact, with the new surface being walk 2's six folds.
...
| P3 | the lint's contract — ⛔ **PINNED, because this pin is a claim about a FILE's contents** (lens 1, 1.3): `scripts/walk_register_lint.py` sha256 `80f7580a24ab65a3a96bd9658e77722de3ecbcab64d8f8c42d31d85e03e1c182` — without it P3 goes stale silently the next time that file is edited, and this plan's whole premise rests on it. ⚠️ **RE-PINNED 2026-09-06, and the pin DID ITS JOB.** It was `e50b63a1…` when Step 1 ran; `2a1e153` and `9c77f95` landed 25 min and 4 h later, and a re-run HALTED on the mismatch. ⛔ **The pin protects a CLAIM, not a digest — the claim was re-tested against the new file BEFORE the digest was moved,** and it HOLDS: `REQUIRED_COLUMNS` is byte-identical, the six value-reading sites are unchanged (`pre_fold_text` truncation, `finding`/`resolution` ellipsis marker, and a generic emptiness test), `2a1e153` reads the column NAME `id` from a header, and `9c77f95`'s `coverage_basis` counts rows and scrapes prose — neither reads the id cell's VALUE. The lint still has no notion of finding identity. ⛔ Re-pin only after re-testing the claim; a bare digest bump silently converts a premise check into a rubber stamp | `REQUIRED_COLUMNS = ["id","walk","lens","sub_question","origin","finding","pre_fold_text","resolution"]` — ⛔ **`"id"` is a name checked for PRESENCE; the cell's VALUE is never read** | `sed -n '31,34p' scripts/walk_register_lint.py` |
...
- ACID:                w1 2 folded — instruction 2 / record 0; w2 1 folded — instruction 0 / record 1.
⚠️ **Record reconciliation, walk 3 lens 1:** the walk-2 lines above sum to **6** folds while the register carries **5** `w2-` rows — the ACID finding (w2-6) was folded into this body and its battery line but never given a register row. ⛔ **That is thread 135's own subject occurring inside the diagnostic investigating it**, one walk after the body block was written to fix the previous record failure. Row added at walk 3; the counts now reconcile at 6.
⛔ **Do NOT rely on the manifest's `coherence` field to catch a Cycle Log that lags its register (walk 2, ACID 5.4).** `cycle_check._compute_coherence` DOES reconcile register rows against body walks — but `total_walks = max(walk_data) if walk_data else 0` and `if total_walks == 0: return "N/A"`, so **the check is disabled by exactly the condition it should catch**, and it runs only on `--emit-manifest`, at freeze, never per walk. ⚠️ Third instance in two days of one shape: a `fold_check` baseline that cannot observe the fold it was taken from, an escalation ladder with nothing to evaluate, and now a coherence check that returns N/A on the absence it exists to detect. **The record's currency is maintained by discipline alone; this cycle proved that discipline fails.**
...
>
> ⚠️ **Scope and Deposits must list the SAME four paths (checked 2026-09-05).** The raw `.txt` was in Deposits and NOT in Scope — a deposit the agent is not scoped to write, which `scope_check` refuses or which lands outside declared scope. Re-check both lists together after any edit to either.
>
...
>
> ⛔ **SELF-INCLUSION — THE CENSUS COUNTS ITS OWN REGISTER (walk 2, lens 3, 3.3).** This plan's walk register lives in the very directory being censused and matches `walk-register-*.md`; the population moved **172 → 173** the moment it was created, and its row count changes at every lens fold. Its research note deposits into that same directory. ⛔ **The instrument must EXCLUDE `walk-register-register-coverage-2026-09-04.md` BY THAT EXACT NAME and report the exclusion in the note** (walk 3, lens 1: the draft said "by name" and named nothing, leaving an executing agent to infer which register is its own), and Item 2b's HEAD pin must be read BEFORE this cycle's own commits or it fires on itself. ⚠️ Same family as two traps hit on 2026-09-04: a `fold_check` baseline compared against the state it was taken from, and a `plan_lint` pin on the file under edit — **a measurement whose subject includes the measurer.**
>
...
>
> ⛔ **WRITE INCREMENTALLY — a killed run must leave partials.** Create the raw `.txt` FIRST and append each measurement AS IT IS ESTABLISHED, before any note or dev-log is written. ⚠️ **This is not caution, it is measured:** a cold-panel seat's task died on 2026-09-04 after finishing its work, and its 16 findings survived ONLY because its template mandates appending as established; the same day, a background agent's transcript was lost with nothing on disk. ⛔ Do not batch every output to the final Items.
>
