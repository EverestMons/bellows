...
- **Row 6** — `tools/clear_plan.py`: `--override-gate <plan-id-or-slug> <step> <gate> --ref "<justification>"` subcommand beside `--release-class-hold`: id-native → UPDATE the gate_events fail row to `overridden=1, override_ref=<ref>` (refuse if no matching fail row); slug-only → the design's request-file fallback with the ledger note; prints what it changed; the deliberate human act, mirroring the release arm's posture.
