...
- ⚠️ **Neither ARM changes.** On any step where the two predicates agree, the emitted WARN text must be byte-identical to today's, and the evidence is a full-corpus before/after diff rather than a spot check. ⚠️ **What that check can actually catch is an agent who tidies the arms while inside the block** — under a correct edit it cannot fail, because the arm bodies are not in the edit set. It is a guard, and the "What this plan does NOT do" bullet says so in the same words.
...
**Walks:** 7 (walks 0–7 complete, plus one cold scout round and one declared NON-WALK; the cycle is open). ⚠️ **RE-LABELLED, not tidied:** the §2.7 class enumeration was first recorded as "walk 6", which was wrong — §2.7 fires it INSTEAD of walking and it discharges nothing, so it is a declared non-walk and the pass after it is walk 6. The correction also removes a spurious `cycle_check` `ESCALATE:yield-rising` that the mislabelling manufactured (a 1-instruction "walk" followed by a 2-instruction one). The enumeration's own finding is folded and counted in the totals either way.
- Weak spots:          w1 2 folded — instruction 2 / record 0; w2 2 folded — instruction 1 / record 1; w3 4 folded — instruction 4 / record 0; w4 1 folded — instruction 1 / record 0; w5 3 folded — instruction 2 / record 1; w6 1 folded — instruction 1 / record 0; w7 2 folded — instruction 2 / record 0.
- Destruction:         w1 1 folded — instruction 1 / record 0; w2 1 folded — instruction 1 / record 0; w3 1 folded — instruction 1 / record 0; w4 1 folded — instruction 1 / record 0; w5 2 folded — instruction 0 / record 2; w6 dry; w7 1 folded — instruction 1 / record 0.
- Vulnerabilities:     w1 2 folded — instruction 2 / record 0; w2 2 folded — instruction 2 / record 0; w3 1 folded — instruction 1 / record 0; w4 1 folded — instruction 1 / record 0; w5 1 folded — instruction 0 / record 1; w6 dry; w7 1 folded — instruction 1 / record 0.
- Integration-record:  w1 3 folded — instruction 2 / record 1; w2 2 folded — instruction 1 / record 1; w3 3 folded — instruction 3 / record 0; w4 dry; w5 4 folded — instruction 0 / record 4; w6 1 folded — instruction 1 / record 0 (the §5 conformance pass); w7 4 folded — instruction 1 / record 3.
- ACID:                w1 2 folded — instruction 2 / record 0; w2 2 folded — instruction 2 / record 0; w3 1 folded — instruction 1 / record 0; w4 1 folded — instruction 1 / record 0; w5 dry; w6 dry; w7 1 folded — instruction 1 / record 0.
...
**The CLASS ENUMERATION §2.7 mandates — a DECLARED NON-WALK, discharging nothing —** ran between walks 5 and 6, fired because one class — *a claim whose truth depends on an unstated probe scope or an unnamed population* — had produced findings on four consecutive passes (w2-1, w3-6, the scout's S-1/S-2/S-9, and w5-3/6/7/10). Every value the plan pins in one place and cites in another was listed and re-measured by RUNNING its producer in a single script: **57 pins, 56 OK, 1 MISMATCH** — walk 3's duplicate-heading justification, false on the fence-stripped population and true only on the raw one, which is the same three plans the scout's finding names. The list is the deliverable and the mismatch was the class caught on the enumeration's first run.
**Mechanical conformance pass (§5), run at shape stability — walk 6, not first at deposit:** `plan_lint` **exit 0, 0 FAIL** (last run at the freeze battery); `propagation_check` **exit 1, 136 divergences**, and every one classified before the close rather than shipped unexamined. **Twenty were a real declare-once violation and were removed** — two supersede lists restated five pin values verbatim, which is the only place in the plan where a stale copy could mislead an executing agent; both now POINT at the pin table (136 → 116). ⚠️ **The remaining 116 are the checker's symbol-extraction class — thread 96's rider — and the demonstration is mechanical, not asserted:** the checker treats each pin ID as a declared symbol and every numeral in that pin's row as its value. `P13` is the SUITE-BASELINE pin; its row yields the numeral `67` out of `58.67s`, and the checker then reports `P13 = 67 restated` at every mention of the 67 false positives — two unrelated quantities. `P18`'s row yields `230, 582, 885, 132, 226, 215, 236, 607`, all of them line references, each flagged wherever that numeral appears. Over a 19-pin table in a plan whose subject IS counts, that is what the detector produces. Zero real restatement divergences remain.
...
**Closing:** the cycle is open; walk 7 left six instruction-class findings; no close claimed.
...
>
> **Item 6 — run the AFTER collection with the SAME committed script and the SAME command Item 1 used**, and pair it with Item 1's recorded BEFORE. ⚠️ **Only the AFTER side is produced here.** The WARN-line BEFORE set cannot be re-derived once the edit has landed — that is why Item 1 collects it first and writes it to the dev log verbatim. The predicate half re-runs whole, because it models both predicates and does not depend on which one the code carries. This completes P8. Emit, in full and verbatim in the dev log:
