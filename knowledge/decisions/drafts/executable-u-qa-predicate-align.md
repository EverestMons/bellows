...
>
> 1. **Every fixture needs the check-(c) banner pair somewhere, or the lint exits 1** on `FAIL: (c) QA banner pair — missing: banner, PASSED line` and the test's exit-code assertion is about (c), not about (u). Measured: a fixture without it exits **1**; the three existing (u) fixtures carry it inside their QA step, which is why they exit 0.
...
> 4. **the header arm without the heading:** `qa_steps: 2` with `## STEP 2 — DEV` (no `qa` in the heading) → the WARN still fires, because the gate's primary arm returns the `qa_steps` membership and never consults the heading. Guards against a mutant that keeps only the keyword fallback. ⚠️ **Two measured side effects of this fixture, so they do not read as failures:** without the banner pair it exits 1 on `FAIL: (c)` (constraint 1 — add it), and it also emits `WARN: qa_steps lists step 2 but step 2 is not QA-labeled — it will be gated as QA (plan-133 trap)` from the sibling cross-check at `:346`. **That sibling WARN is corroboration, not noise** — it and (u) now agree that step 2 is gated as QA, which is the whole point of the alignment. Do not suppress it and do not assert on total output.
