...
> 3. **The resolution, measured and working: put the banner pair ABOVE `## STEP 1`.** Check (c) reads the whole plan text; `gates._extract_step_text(plan_text, 2)` does not reach above the step heading. Verified on a constructed fixture: `exit 0`, no `FAIL`, `'Rule 20' in step-2 text → False`, `gate says step 2 is QA → True`. ⚠️ **Never place it after the last step** — a trailing block lands inside the final step's span, which is the same absorption that makes the real plans' trailing fence work, and here it would reintroduce the token.
