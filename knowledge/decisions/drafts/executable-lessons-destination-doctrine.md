...
**Date:** 2026-09-03 | **Project:** bellows | **Tier:** Small | **Dispatch Mode:** bellows | **cycle_tier:** T2 | **Test Scope:** none (doc-only — ⛔ **scoped claim, corrected at seat 1 (S1-7):** no code path reads the CONTENT of `PLANNER_TEMPLATE.md`, `wrap.md` or `eluvian.md`; **`LESSONS.md` is NOT in that set** — 9 python readers exist, including `forge_lessons/scripts/detect_learned.py`, `bellows/scripts/check_backlog_freshness.py` and `bellows_root.py`, and this plan itself states twice that `LESSONS.md` is forge-ingested. The append is additive and changes no line any reader keys on, which is the actual safety argument; the blanket claim was false. `100027` scoped this to ONE file precisely because it enumerated its readers; QA proves the gates unchanged on a shipped plan and deposits its raw probes as `.txt` evidence, which `qa_test_result` will refuse to certify having no pytest summary to parse: the pre-declared benign gate failure of Step 2's gate note, overridden by the Planner with reference to it) | **Execution:** Step 1 (DEV) → Step 2 (QA) | **qa_steps:** 2 | **pause_for_verdict:** always | **known_failures:** 0
...
`drafts/executable-memory-destination-and-gate.md` carries a **RE-DRAFT** banner. Its premise — *"the rule is already in the code"* — was measured FALSE by a T2 cold panel: `wrap_check.py:344` is `if "class:" not in _head` over the first 600 bytes of the whole file, a substring check. ⛔ **Corrected at seat 1 (S1-2): do NOT cite the `class: banana` demonstration as fact.** That input was CONSTRUCTED and the corpus does not produce it; `governance/knowledge/research/thread-114-rederivation-2026-09-04.md` — which this plan cites — states it *"remains constructed and invalid"* and records that what is VERIFIED is **inertness**: `MEMORY` is not a git repo here, `porcelain(MEMORY)` returns `[]`, and `wrap_check.check()` returns **0 fails** across 12 entries carrying no `class:`, 9 of them modified after the gate shipped. ⚠️ The substring weakness is almost certainly real and is UNREACHABLE on this machine, so it remains undemonstrated from the corpus. On the 139-entry shop corpus, 11 entries carry a class and **all 11 are `stale`**.
...
| P13 | §2.0 (3) occurrence counts — ⛔ **OCCURRENCES, not lines (corrected at seat 1, S1-3)** | `three destinations` in PT: **2** (one live, one History row) · `memory repo` in PT: **4 occurrences on 3 lines** — ⚠️ **TWO of them are on the edited line 2094 itself**, at cols **2183** (historical narrative — do not touch) and **2576** (`…go to the Planner's memory repo. Three guards:` — **the live routing rule, which MUST change**), plus `:2267`/`:2272` History rows. ⛔ **`grep -c` counts LINES and hid the second site**; this is the same two-on-one-line defect the plan already caught for `three destinations`, un-propagated to the sibling token · `working-pattern` in wrap.md: **1** · `working patterns` in eluvian.md: **1** | `grep -c` each |
...
⛔ **Restored at seat 1 (S1-14) — this section was dropped whole from the clone origin, and with it the invariants below had no home.** Distinct from "What this does NOT do": those say what is not touched, these say what must still be TRUE afterwards.

- ⚠️ **TWO REPOSITORIES, ONE STEP.** Governance by absolute path with `git -C "$GOV"`, never `cd`; commit by EXPLICIT PATHSPEC (this plan's own walk register lives in the governance repo and will be dirty); ⛔ **bellows LAST**.
- ⛔ **No landed text carries a Rule 20 hedging keyword.** The receipt rows DESCRIBE the rewritten sentences; they never quote a body containing one.
- ⛔ **`wrap.md:69`'s co-tenant survives byte-identical** from `**Then add a line to \`shop_next_session.md\`:**` onward — `wrap_check`'s `[3b/lessons]` blocks the wrap on the line it introduces (walk 2, w2-2).
- ⛔ **`eluvian.md:31`'s second clause survives** — `on a path to hardcoding — systems must not RELY on it` is a standing invariant this plan does not supersede (walk 2, w2-3).
- ⛔ **`LESSONS.md:4447`, its `[status: implemented]` marker, its body and its baton clause stay byte-identical.** The supersede is an APPEND; `git diff --numstat` shows lines added and **zero deleted**.
- ⛔ **Step 1's Deposits block keeps all four doctrine paths.** Trimming it re-opens S1-5 — the depositor's fallback would derive `app-feature` and auto-clear.
- ⛔ **Never run any `checkout --` on an A0 halt** except a named recipe whose discriminator holds; a pin mismatch otherwise means the file carries changes this plan did not make.
- **`known_failures: 0`, no suite.** The QA `.txt` evidence is `probes-raw.txt`; a doc-only plan that invokes a suite declares a scope it does not have.
...
**Panel: FULL form (Fork C)** — the RE-DRAFT-adjacent signal fires it; the small form's proxy admits only mechanized clones and this plan is a rebuild after a RE-DRAFT verdict. Scout (walk 0) + DISCOVERY → EXECUTION → CAPSTONE, sequential. ⛔ **Fired EARLY, before §2's bar — so the judged-stop relaxation is gone and this cycle closes only on a FULLY DRY walk.**

**Walks:**
- Weak spots:          w1 2 folded — instruction 1 / record 1 (Item 2's false enforcement claim; the non-reproducing column pin); w2 1 folded — instruction 1 / record 0 (P2's two-line anchor); seat 1 5 folded.
- Destruction:         w1 1 folded — instruction 0 / record 1 (the live-at-write hooks declaration); w2 2 folded — instruction 2 / record 0 (the `[3b/lessons]` co-tenant; the `eluvian.md` invariant); seat 1 2 folded.
- Vulnerabilities:     w1 dry; w2 dry — ⛔ **no cold execution yet; the EXECUTION seat is the first to RUN this plan's probes.**
- Integration-record:  w1 dry (P7-vs-Item-2 is counted once, under Weak spots); w2 dry; seat 1 6 folded (the clone-diff's dropped machinery).
- ACID:                w1 1 folded — instruction 1 / record 0 (the LESSONS.md guard/write window); w2 1 folded — instruction 1 / record 0 (the form-not-truth post-condition); seat 1 dry.
⚠️ **Counts corrected before this line was written: a finding spanning two lenses is counted ONCE.** w1-4 (weak-spots + integration-record) and w2-4 (weak-spots + ACID) were each double-counted in a first draft of this block, inflating both walks. Totals now reconcile with the register: **walk 1 = 4 findings, instruction 2 / record 2; walk 2 = 4 findings, instruction 4 / record 0.**
- Cold panel — scout at walk 0 (folded pre-walk-1). **DISCOVERY closed — 18 findings, HIGH 8 / MED 8 / LOW 2, all eight HIGHs author-verified and reproduced; ~170k tokens.** Two were SAFETY: the deposit would have derived `app-feature` and auto-cleared (S1-5), and an undefined `$GOV` silently sent the A0 re-entry ladder down FRESH (S1-8). Clone-diff: 30 hunks, **10 dropped**, cross-join mapping 14 of 18 findings to a dropped or mis-adapted hunk. EXECUTION and CAPSTONE pending.
...
> ⛔ **DEFINE THE ROOTS FIRST — `$GOV` was used twice and defined ZERO times (seat 1, S1-8, a HIGH).** Both parents define it three times; this clone dropped the block. Same invocation as every use:
> ```
...
> ```
> ⛔ **Why this is safety-bearing and not hygiene:** with `$GOV` unset, `git -C "" log -1 --format=%H -- PLANNER_TEMPLATE.md` prints **nothing and exits 0** — measured. The ladder below reads that output to choose its state, so an unset root sends it silently down **FRESH** no matter what is true, defeating the one mechanism that prevents re-applying a doctrine edit. ⛔ **Assert the read is NON-EMPTY before branching.**
>
> ⛔ **A0 RE-ENTRY LADDER — three states, because the ordering above creates a half-landed one.** Before editing, read `git -C "$GOV" log -1 --format=%H -- PLANNER_TEMPLATE.md` (⛔ **empty result → NONE-MATCH, never FRESH**) and the file's version string:
...
>
> **Item 1 — re-derive P1–P14 and HALT on mismatch.** ⛔ Line numbers move: thread 91 recorded `:2028` on 2026-09-02 and it was `:2094` a day later. Re-derive every one by grep. ⛔ **The four-case probe is a NULL DISCRIMINATOR here and must not be used as the HALT test (seat 1, S1-1, a HIGH).** On this machine `porcelain(MEMORY)` returns `[]`, so the `[4/memory]` loop body is unreachable and the gate cannot refuse in EITHER state of the world — the probe returns the same answer whether or not thread 114 has landed. ⛔ **Test the MECHANISM instead:** re-read `wrap_check.py`'s `[4/memory]` block and HALT only if it no longer derives its candidate set from `porcelain(MEMORY)` (i.e. thread 114's fix has changed how entries are detected). Re-derive `porcelain(MEMORY)` and the classless-entry count as evidence, and record both.
>
...
> ⛔ **Give "says so" a FORM, or the amended rule is unactionable.** ⚠️ **This instruction previously justified itself by saying `wrap_check` fails any memory entry whose first 600 bytes lack `class:`. That claim is FALSE on this machine and it violated P7 — folded at walk 1.** Measured 2026-09-04: `MEMORY` resolves to a directory that is NOT a git repo, so `porcelain(MEMORY)` returns `[]`, the classless loop never executes, and `wrap_check.check()` returns **0 fails** over 12 entries carrying no `class:` at all — 9 of them modified after the gate shipped. ⛔ **The gate is INERT here, not merely weak**, so it requires nothing and cannot be cited as requiring anything (P7, thread 114, and the re-derivation at `governance/knowledge/research/thread-114-rederivation-2026-09-04.md`). **The form is still owed — for the shop machine, where the memory directory IS git-backed and the gate does fire, and for a reader who needs to know what 'says so' looks like — but it is owed on those grounds, not on an enforcement that does not exist.** A Planner who follows this step and writes only prose fails the wrap against a vocabulary no document they read contains. **Point the reader at P8's audit** (`governance/knowledge/research/memory-to-system-audit-2026-08-25.md`), which is the vocabulary's origin and carries its semantics and a worked classification. ⛔ Do NOT re-define the vocabulary here — cite the audit. ⚠️ **And do not treat the admission test as unwritten (seat 1, S1-10): thread 91 already supplies it** — memory holds *"identity, working preferences, pointers"*, with dedup and class-not-narrative as the filters and a recurrence becoming a **rider on the existing entry**. ⛔ That is an ADMISSION test (what may enter memory); P8's vocabulary is a RETIREMENT test (what to do with what is already there). Routing the reader only to P8 silences thread 91's rule, including the rider convention. ⚠️ Keep the second routing test ("does this destination feed a system that ACTS on it?") — it still separates forge-ingested `LESSONS.md` from the glossary; re-word only its examples. ⛔ **Fix the dead path in the sentence you are rewriting** (P9): the glossary is at `$ELUVIAN_WRAP_ROOT/GLOSSARY.md`, matching what `wrap.md` already does. ⛔ Do NOT touch the other 28 `Developer/GitHub` references, and do NOT edit the History rows at `:2267`/`:2272` (P5). ⛔ **Guard (a)'s state enumeration is on this same line and is measurably incomplete (seat 1, S1-4)** — it omits `verdict-pending-*`, a live state the walk-0 scout observed. ⚠️ **Fix it in the same edit or DECLARE why not**: `100027` carried a §6 coordinate-doctrine-and-gate line for exactly this case, and this clone dropped it. Leaving a known-wrong enumeration on a line you are rewriting ships a fresh defect the way P9's dead path would.
>
...
> - ⛔ **House format is two markers, both measured and both currently unnamed (seat 1, S1-9):** the entry's heading carries **`[status: pending]` and only that** (`wrap.md:60-61` — the marker is a projection of the forge DB and no other value may be hand-written), and the entry ends with a **`**Tag:**` line** (`PLANNER_TEMPLATE.md:2094`, house format). ⚠️ Corpus size re-measured: **5,925 lines**, not the "5,700" this plan states.
...
>
> **Item 7 — ⛔ `GATE_BASELINE` — capture the BEFORE (seat 1, S1-6; carried by clone origin `100027`, dropped here).** QA Item 3 compares gate verdicts "before and after" and **no step captured a before**. Run `plan_lint` and `cycle_check` against a SHIPPED plan (name it, and use the same one at QA) BEFORE any edit in this step, and record their verdicts verbatim in the dev-log. ⚠️ **This control cannot fail by construction for three of the four files** — no gate reads their content — which is exactly why its absence was invisible; capture it anyway, because "cannot fail" is the claim under test. ⛔ **Do not name `gates.check` as the instrument**: its signature is `gates.check(parsed, plan_text, step_number, project_path, …)` and it is not invocable against a plan path.
>
> **Item 7b — dev-log**, recording all four surfaces' prior text verbatim, the supersede, the `GATE_BASELINE` verdicts, and BOTH commit shas.
>
> **Deposits:**
> - `knowledge/development/dev-log-lessons-destination-doctrine-2026-09-03.md`
> - `/Users/marklehn/Developer/eluvian-governance/PLANNER_TEMPLATE.md`
...
> - `hooks/commands/eluvian.md`
>
> ⛔ **The four doctrine files are listed here DELIBERATELY and must not be trimmed to "just the dev-log" (seat 1, S1-5 — a HIGH).** `depositor._parse_plan` returns `(writes, reads, declared_class)`; the **Scope** block feeds *reads*, and while the Cycle Manifest is still the BAR_MET placeholder, *writes* fall back to `gates._extract_plan_required_deposits` — i.e. to THIS block. Measured before the fold: `writes=1` (the dev-log alone, under `knowledge/`) → `_assign_class` → **`app-feature`, which AUTO-CLEARS**, dispatching a four-file two-repo doctrine edit with **no human release act**. ⚠️ **That is verbatim the 2026-09-03 failed-open deposit.** Both parents (`100027`, `100026`) list the edited doctrine file in Deposits and therefore derive `shop-infra` even on the fallback path. ⛔ **Re-run the assigner after any edit to this block** — P11 is only true while this block is right.
>
...
>
> ⛔ **The override ACT was declared but never written down (seat 1, S1-18).** It is `tools/clear_plan.py --override-gate <plan-id> 2 qa_test_result --ref <path>`. ⚠️ **`--ref` must point at a COMMITTED path, and the justification must be committed BEFORE the override runs** — `--override-gate` is idempotent and refuses once applied, so the reference is write-once and uncorrectable (`LESSONS.md` 418, thread 123, and the `CLAUDE.md` runbook). A scratchpad path becomes a dead pointer the moment the session ends.
>
...
>
> **Item 5 — hygiene + receipt:** numstat vs BOTH DEV commits; toplevel; reflog `-n 4` **in BOTH repos, each named explicitly** (seat 1, S1-16 — an unqualified `reflog` reads bellows only and is blind to the governance commit) → 0 amends; per-item table; then the QA self-check block inside a Verification-headed section (the 556 placement law).
>
