...
- `hooks/eluvian/wrap_check.py`: `check(session_id: str | None = None)`; `main()` parses `sys.argv[1]` (empty → None); new `[2r/receipts]` group appended after step 4, implementing **C-1's corrected semantics** — blocking arm (own receipts vs clearances/hold-sidecars), warning arm (24h lookback, any-session), SKIP arms (`no session_id`, `receipts/ absent or unreadable`, `lifecycle.db unreadable` — each with its named printed line), every arm's errors handled in-step. ⚠️ **Channel discipline: ONLY the blocking arm appends to the `fails` list; warnings and SKIP notes `print()` directly from the step and never enter `fails`** — a warning that lands in `fails` blocks the wrap and violates C-1. **Hold-sidecar location:** mirror `project_done_dirs()`'s existing idiom — `ROOT.glob("*/knowledge/decisions/*.hold.json")` plus bellows' own decisions dir — never a config.json read (wrap_check has none today; keep it that way). **The warning arm consults BOTH `receipts/` and `receipts/archived/`** — a retired receipt still proves its deposit was armed, and ignoring archived/ makes every closed-before-wrap plan a false warning. lifecycle.db opened read-only (`sqlite3.connect(f"file:{path}?mode=ro", uri=True)`).
