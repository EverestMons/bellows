...
- `tests/test_wrap_receipts.py`: `[2r/receipts]` pass (own receipts all matched); blocking arm fires on an own-session receipt matching nothing **whose `armed_at` is aged past the grace window; the pending-evaluation grace — an identical matchless receipt with fresh `armed_at` produces a note, not a failure**; warning arm on a receipt-less clearance (non-blocking — `check()` returns no failure for it); SKIP arms (no session id / no dir / unreadable DB); malformed receipt JSON → warning, no exception; **anti-foreign-block: a foreign session's receipt-less state never adds a failure**; **the blast-radius arm: clearances present + receipts empty + session id given → warnings only, zero failures**; `_retire_receipts` moves matching files, ignores missing, never raises; **the clear-tool release arm (tmp tree + tmp DB): a `class:shop-infra` hold with passing gates → clearance row `cleared_by='clear_tool'` + bare rename; a failing gate → refused, still held; a non-class hold reason → old rename-to-ready path unchanged**. ⚠️ ALL in tmp dirs/DBs.
