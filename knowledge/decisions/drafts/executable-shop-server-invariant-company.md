...
**Depends on:** the CEO, 2026-09-02 ("every machine becomes a shop … The mini is our server which should be the only difference"; "Both"); plan A `shop-server-invariant-sketch-2026-09-02` (held today; this plan's A0 HALTs until its addendum is in the live sketch — the sentence cites it); tuyere thread 86; `Done/executable-100008.md` in forge_lessons (the clone origin and the newest same-class plan: doctrine prose in a governance file by a COMMITTED builder from a project worktree, `git -C` never `cd`; closed 2026-09-01T19:58:22Z by `lifecycle.db`); the builder `governance/knowledge/decisions/drafts/build-shop-server-invariant-company-2026-09-02.py` (committed; digest below). Walk register: `/Users/marklehn/Developer/eluvian-governance/governance/knowledge/research/walk-register-shop-server-invariant-company-2026-09-02.md`.

**Tier computed, not judged (§1):** **T-6 fires** — `COMPANY.md` is doctrine (the handbook that defines the shop) → **T2**. T-1 fires (two repos). T-8 fires (a clone by kind of 100008). T-2/T-3/T-5 no. **Panel form — the magnitude proxy, measured at walk 0 (all three must hold): (i) structure-clone** — the parent is 100008, closed 2026-09-01T19:58:22Z, and both walk-0 clone-diffs ran (against 100008's final text and its register); **(ii) mechanized edits** — every mutation flows through the committed builder, whose three anchors are count-1-asserted BY EXECUTION and whose seven post-conditions are asserted on the output before any write (proven at walk 0: a scratch build, three refusals, a 6-line diff of exactly three changed lines); **(iii) scripted probes** — every post-condition token measured 0 against the pre-edit file. **→ SMALL form: the walk-0 scout, then EXECUTION, then CAPSTONE.** The CEO may call the full form.
...
- **E3** — `**Last Updated:** 2026-08-24 — added `tuyere` to Active Projects (CEO-directed)` → `**Last Updated:** 2026-09-02 — the shop/server invariant: a machine runs the shop, the mini is the server, and that is the only difference (CEO-directed)` (line 4, count 1).

The builder reads the file as bytes, refuses CRLF, asserts each anchor's count is 1, asserts the output tokens are absent in the input, applies the three edits, asserts seven post-conditions on the OUTPUT (the E1 sentence 1, `A MACHINE runs the shop` 1, `**Version:** 2.8` 1, `**Version:** 2.7` 0, the new Last-Updated line 1, the old 0, `the only permitted difference between shops` 1), asserts the line count is unchanged (350) and the byte delta equals the three edits, and only then writes — to a SCRATCH path it refuses to let be the live file or anything under the governance root. Apply = copy the proven scratch output over the live file, then re-measure the live file.
...
| P4 | **`BUILDER`** — on-disk digest = committed blob | `6313c4efb2b1c0d2` (first 16 of sha-256, both) | `shasum -a 256 <builder> \| cut -c1-16`; `git -C "$GOV" show HEAD:governance/knowledge/decisions/drafts/build-shop-server-invariant-company-2026-09-02.py \| shasum -a 256 \| cut -c1-16` |
| P5 | **`DRY_RUN`** — the builder's success line and diff shape | `BUILT: <out> lines=350 delta_chars=507 edits=3 post=7/7`; `diff COMPANY.md <out> \| grep -c '^[<>]'` → 6 | as stated, scratch-only |
...
>
> **A2 — dry-run, scratch→scratch:** `S=/tmp/ssic-scratch; rm -rf "$S"; mkdir -p "$S"; python3 "$B" "$GOV/COMPANY.md" "$S/COMPANY-out.md"; echo "builder_exit=$?"` → P5's `BUILT:` line verbatim and `builder_exit=0`; `diff "$GOV/COMPANY.md" "$S/COMPANY-out.md" | grep -c '^[<>]'` → 6; `wc -l "$S/COMPANY-out.md"` → 350. Then the refusals (each must print `BUILDER REFUSED` and exit nonzero; quote each): `python3 "$B" "$GOV/COMPANY.md" "$GOV/COMPANY.md"` (out == in); `python3 "$B" "$GOV/COMPANY.md" "$GOV/scratch-out.md"` (under the governance root); `python3 "$B" "$S/COMPANY-out.md" "$S/COMPANY-out2.md"` (already built: anchor count 0).
>
> **A3 — apply and measure the LIVE file (Task C):** `cp "$S/COMPANY-out.md" "$GOV/COMPANY.md"`; then from the live file: P3's three tokens → 1 each; `**Version:** 2.7` → 0; the old Last-Updated line → 0; P2's E1 anchor → 1; `wc -l` → 350; `git -C "$GOV" diff --stat -- COMPANY.md` → 3 insertions, 3 deletions (state it); `cmp "$S/COMPANY-out.md" "$GOV/COMPANY.md"` → silent. Commit: `git -C "$GOV" add COMPANY.md && git -C "$GOV" commit -m "[<id from your plan filename>] COMPANY.md v2.8: a machine runs the shop, the mini is the server, the only permitted difference (CEO 2026-09-02)" -- COMPANY.md`; `git -C "$GOV" log --oneline -1 -- COMPANY.md` → that commit. Do NOT push.
>
...
> - **Item 2 — the builder, by a second pair of hands (the parent's Item 3):** `C=$(git -C "$GOV" log -1 --format=%H -- COMPANY.md); Q=/tmp/ssic-qa; rm -rf "$Q"; mkdir -p "$Q"; git -C "$GOV" show "$C^:COMPANY.md" > "$Q/COMPANY-pre.md"; python3 "$B" "$Q/COMPANY-pre.md" "$Q/COMPANY-out.md"; echo "builder_exit=$?"; cmp "$Q/COMPANY-out.md" "$GOV/COMPANY.md" && echo BYTE_IDENTICAL` → the `BUILT:` line, `builder_exit=0`, `BYTE_IDENTICAL` (the pre-edit blob rebuilt by the committed builder equals the live file byte for byte). Then P4 (on-disk digest = committed blob) and the three refusals of A2 reproduced (quote each).
