...
**D-5 — activation + coordination.** The change is daemon-code: INERT until restart. ⚠️ The pending restart ALSO activates E3's `_retire_receipts` — one deliberate restart, two arcs' activation; specify the post-restart canary for BOTH (a continue on a synthetic failed-gate verdict-request in a scratch-safe watched dir → observe the refusal disposition; a plan close → observe receipt retirement), each canary SAFE-IF-MISFIRED per the E2 canary lesson. Name the shared-file fence: bellows.py moved under E3 this same day — the executable X-pins the blob and HALTs on drift.
