...
The builder reads the file as bytes, refuses CRLF, refuses an already-built input, refuses a missing input, asserts the five anchors (E1's head and tail each count 1 and ending the same line; E4 and E5 as whole lines), applies the edits, asserts twenty post-conditions on the OUTPUT (the new sentence once; the four corrected clauses' tokens once each; the retired class-list literal 0; `HARD-HOLD` and `HOLD-AND-REPORT` at 2 each — the 2.17 row and the new 2.24 row, the sentence's own occurrence gone; the 2.24 version 1 and the 2.23 version 0; the 2.24 row 1 and the 2.23 row surviving 1; the example's new `class:` and `validation:` lines 1 each and their old forms 0; three section headings once each), asserts the line delta against `EXPECTED_LINES = 1` and the byte delta against `EXPECTED_BYTES = 4638`, and only then writes — to a SCRATCH path it refuses to let be the live file, anything under the literal governance root, or anything under the INPUT's git toplevel (layout-independent). Apply = copy the proven scratch output over the live file, then re-measure the live file.
...
| P3 | **`TOKENS`** 0 before | `ten REQUIRED fields` · `three OPTIONAL fields` · `one of the FOUR values` · `compares ONLY the declared` · `2.24 (2026-09-02)` · `slug dc-manifest-sentence-2026-09-02` — each 0; invariants: `HARD-HOLD` 2, `HOLD-AND-REPORT` 2, `{read-only, governed-tooling, register-writing}` 1, `class: governed-tooling` 1, `class: shop-infra` 0, `propagation_check=CLEAN` 0 | `/usr/bin/grep -cF` |
| P4 | **`BUILDER`** — on-disk digest = the blob at its OWN last commit | `e12135909645f27f` (governance `95dd9d56`) | `shasum -a 256 <builder> \| cut -c1-16`; `BC=$(git -C "$GOV" log -1 --format=%H -- governance/knowledge/decisions/drafts/build-dc-manifest-sentence-2026-09-02.py); git -C "$GOV" show "$BC:governance/knowledge/decisions/drafts/build-dc-manifest-sentence-2026-09-02.py" \| shasum -a 256 \| cut -c1-16` |
| P5 | **`DRY_RUN`** — the success line, numstat, sizes | `BUILT: <out> edits=5 lines+1 bytes+4638 post=20/20`; `git diff --no-index --numstat -- "$GOV/DRAFTING_CYCLE.md" <out>` → `5	4` (exit 1 is the differing state — read the numbers); `wc -l <out>` → 370; `wc -c <out>` → 169,224 | as stated, scratch-only |
...
>
> **A2 — dry-run, scratch→scratch (LITERAL paths, no variables across compounds):** `rm -rf /tmp/dc67; mkdir -p /tmp/dc67; python3 "$B" "$GOV/DRAFTING_CYCLE.md" /tmp/dc67/DC-out.md; echo "builder_exit=$?"` → P5's `BUILT:` line verbatim, `builder_exit=0`; `git diff --no-index --numstat -- "$GOV/DRAFTING_CYCLE.md" /tmp/dc67/DC-out.md` → `5	4`; `wc -l /tmp/dc67/DC-out.md` → 370; `wc -c` → 169224. Refusals (each `BUILDER REFUSED`, nonzero; quote each): `python3 "$B" "$GOV/DRAFTING_CYCLE.md" "$GOV/DRAFTING_CYCLE.md"` (out == in); `python3 "$B" "$GOV/DRAFTING_CYCLE.md" "$GOV/x.md"` (under a forbidden root); `python3 "$B" /tmp/dc67/DC-out.md /tmp/dc67/DC-out2.md` (already built); `python3 "$B" /tmp/dc67/absent.md /tmp/dc67/DC-out3.md` (input missing).
>
> **A3 — apply and measure the LIVE file (Task C), then commit:** `cp /tmp/dc67/DC-out.md "$GOV/DRAFTING_CYCLE.md"`; then from the live file, counts read never exit codes (`/usr/bin/grep -cF --`): `ten REQUIRED fields` 1 · `three OPTIONAL fields` 1 · `one of the FOUR values` 1 · `compares ONLY the declared` 1 · `{read-only, governed-tooling, register-writing}` 0 · `HARD-HOLD` 2 · `HOLD-AND-REPORT` 2 · `**Version:** 2.24 (2026-09-02)` 1 · `**Version:** 2.23 (2026-09-01)` 0 · `- **2.24 (2026-09-02):** slug dc-manifest-sentence-2026-09-02` 1 · `- **2.23 (2026-09-01):** slug gate2-dc-w28-2026-09-01` 1 (the old row survives) · `class: governed-tooling` 0 · `class: shop-infra` 1 · `validation: cycle_check=BAR_MET, plan_lint=0_FAIL, fold_check=PASS, propagation_check=CLEAN` 1 (the §3 example, the emitter's four pairs); `wc -l` 370; `cmp /tmp/dc67/DC-out.md "$GOV/DRAFTING_CYCLE.md"` silent; `git -C "$GOV" diff --stat -- DRAFTING_CYCLE.md` → 5 insertions, 4 deletions. Any failing → FRESH: `git -C "$GOV" checkout -- DRAFTING_CYCLE.md` + HALT; RE-ENTRY: HALT, no restore. Commit: `git -C "$GOV" add DRAFTING_CYCLE.md && git -C "$GOV" commit -m "[<id from your plan filename>] dc-manifest-sentence: DRAFTING_CYCLE v2.24 — the thread-67 sentence and the §3 example reconciled to depositor.py/plan_lint.py/cycle_check.py (no rule changed)" -- DRAFTING_CYCLE.md`; `git -C "$GOV" log --oneline -1 -- DRAFTING_CYCLE.md` → that commit. Do NOT push.
>
...
> - **Item 1 — the live doctrine file:** `git -C "$GOV" log --oneline -1 -- DRAFTING_CYCLE.md` (the `[<id>]` commit); every Task C count re-run (fourteen probes); `wc -l` 370; `git -C "$GOV" status --porcelain -- DRAFTING_CYCLE.md` → EMPTY.
