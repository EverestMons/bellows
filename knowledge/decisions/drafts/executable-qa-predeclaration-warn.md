...
1. **The check is silent when no test scope is declared at all.** Measured: **53 of 306** QA steps in `Done/` declare no `test_scope` field. (v) speaks only where the author volunteered a declaration, so roughly one QA step in six is structurally out of its reach. Closing that would require inferring test scope from step prose — the judgement this section calls irreducible. ⚠️ **A second, smaller population sits behind the same wall:** 16 `Done/` plans have a header that does not parse at all, hiding a further 4 QA steps from both the funnel and the check — so (v)'s true reach is **253 of 310** QA steps, not 253 of 306. It fails safe (a falsy header yields no test scope, so the check stays silent). Both blind spots are real, bounded, and not addressed here. *(Cold scout S1-3.)*
...
- ⚠️ **No `LESSONS.md` edit — but the record error is ROUTED, not ignored (Rule 102).** `LESSONS.md:5224` still prescribes the REFUTED remedy as the one to prefer: a check that warns when a QA step names no `.txt` deposit. That is the corpus teaching the fix this plan's own premise disproves, and it is the most likely reason check (u) was built that way on 2026-09-02 — a day AFTER the refutation. Under the copy-from-the-corpus law the next author rebuilds (u) again. A doctrine edit is not this plan's scope and is not smuggled into it; **a thread is owed for a struck clause on that bullet**, and leaving it unrouted would be the defect. *(Cold scout S1-4, MED, author-verified at `LESSONS.md:5224`.)*
...
| P6 | the refutation, re-measured | 548's shipped raw-probe evidence file = **63 lines, 0 matches** for `gates._PYTEST_SUMMARY_RE`; positive control fires | the probe in Step 1, Item 1 |
...
| P11 | ⚠️ (u)'s predicate diverges from the gate's | across **861** steps in `Done/`, (u)'s `qa_steps`-or-keyword test disagrees with `gates._gate_is_qa_step` on **74** — **66** where (u) says QA and the gate does not, **8** where the gate says QA and (u) is silent. **(v) must therefore call the gate's predicate directly** | ⚠️ **Reproducible only with the exclusion stated:** count `^## STEP` headers ONLY in plans whose header parses (`gates._parse_plan_header` truthy) — 16 plans do not, and including them gives 868/310/77 (68 FP / 9 silent) instead. **The conclusion is identical under all four variants measured** (raw, fenced-stripped, skip-empty-step-text, header-required), so this pin informs the design and is NOT a halt condition. *(Cold scout S1-2.)* |
...
| P15 | retag blast radius | the paired tag occurs in exactly **two EDITABLE** places: the live source line, and `halted-executable-100022.md:34`, that halted plan's historical spec of what it built. **No test asserts it** — `test_u_no_txt_warns` asserts only the check letter and a fragment of the message. A THIRD literal occurrence exists at `knowledge/qa/evidence/checker-defects-2026-09-02/probes-raw.txt:382` — pasted checker output in a shipped evidence file, out of scope by construction and never edited (cold scout S1-6) | `grep -rn` over `*.py` / `*.md`; read the test |
...
**Walks:** 9 (walks 0–9 complete; the cold scout ran between 8 and 9 and re-opened the walk).
- Weak spots:          w1 3 folded — instruction 2 / record 1; w2 1 folded — instruction 1 / record 0 (step 2's suite command unspecified where the origin gives it exactly); w3 dry; w4 dry; w5 dry; w6 2 folded — instruction 1 / record 1 (the simulated-gate item named a receipt argument it never specified, so the check could not be reproduced); w7 dry; w8 dry; w9 dry.
- Destruction:         w1 2 folded — instruction 0 / record 2; w2 dry; w3 dry; w4 dry; w5 dry; w6 dry; w7 dry; w8 dry; **scout 1 HIGH + 1 MED folded**; w9 dry.
- Vulnerabilities:     w1 1 folded — instruction 1 / record 0; w2 1 folded — instruction 1 / record 0 (a falsy header must degrade, not raise); w3 1 folded — instruction 1 / record 0 (the folded interpreter path was unexecutable from the worktree the same item mandates); w4 dry; w5 dry; w6 dry; w7 1 folded — instruction 1 / record 0 (the evidence directory was never created, so the redirect would fail before any evidence existed); w8 dry; w9 dry.
- Integration-record:  w1 1 folded — instruction 1 / record 0 (the corpus lesson mandating a gate run); w2 1 folded — instruction 1 / record 0 (a separate test module per check family, pinned); w3 dry; w4 1 folded — instruction 1 / record 0 (the exit-code invariant's test had no mutant proving it discriminates); w5 1 folded — instruction 1 / record 0 (three pins pointed at a step label the plan no longer uses); w6 dry; w7 1 folded — instruction 0 / record 1 (a cross-step reference qualified); w8 dry; **scout 3 folded — instruction 1 / record 2**; w9 dry.
- ACID:                w1 dry — the DEV→QA window's guard is inherited and intact; w2 1 folded — instruction 1 / record 0 (a frozen corpus expectation restated as a supersede rule); w3 1 folded — instruction 1 / record 0 (that fold had half-landed, leaving step 2 contradicting step 1); w4 dry; w5 dry; w6 dry; w7 dry; w8 dry; w9 dry.
**Record decay swept at w4 and again at w8:** the walk count lagged twice, and at w4 the correction itself duplicated the per-lens lines — the class §3 names, caught both times by sweeping the log as part of the pass rather than assuming it was swept.
**Walk 8 was an EXECUTION pass, not a read** — every command the final fold set touched was run from a daemon-shaped worktree: the interpreter bind, the directory creation, the suite invocation and its redirect, the target sha, and both count-1 anchor probes. All reproduced. This is the pass that closed the cycle, and the three walks before it had each returned only this cycle's own fold damage.
...
**Cold scout (T1, Planner's call — §2.0):** REPORTED. Lens-4-shaped, no brief layered, run cold on the second machine against `c93d80e`, ~140k tokens / ~20 tool calls. **1 HIGH / 3 MED / 3 LOW — instruction 4 / record 3; no DIRECTION finding** (premise, mechanism and clone origin all independently upheld). **All seven author-verified and folded.** The HIGH is the cycle's most valuable finding and was invisible to eight warm walks: the insertion anchor sits OUTSIDE the neighbouring per-step loop, so the check as specified would have run once against a leaked loop variable and raised on the 25 `Done/` plans that carry no step headings — and the fixture idiom the plan told the agent to clone emits a single step, so all nine tests would have passed anyway. The scout also re-ran the funnel and P6 independently and reproduced both exactly, and said plainly which pins it could NOT verify (no venv on that machine) rather than working around them.
**fold_check re-baselined at the close (INTENDED, declared):** the closing fold cleared one WARN — the one that fires while the log's last event is a fold rather than a dry pass. Interrogated per §3 rather than accepted: it cleared because the condition became FALSE IN FACT (walk 8 is dry across all five lenses), not because the prose was reworded, and `cycle_check` independently moved CONTINUE → BAR_MET on the same edit. Two instruments, one direction.
**Closing (superseded — kept, not tidied):** w8 met the bar on the warm walks; the cold scout then re-opened it with four instruction-class findings, exactly as §2.6 provides. The original close read: w8 met the bar — instruction 0 / record 1: one walk-count lag in this log; 1 of 1 fold-introduced (diagnostic). Walk 8 was an execution pass and returned no instruction-class finding; it restructured nothing, so the convergence clock did not reset. The closing-record re-read was run and itself produced the single record fold above. ⚠️ **The cycle is closed but the artifact is NOT frozen and NOT deposited:** the cold scout convened at walk 2 has not reported, and this record stated that the close would not precede it. The bar is met on the warm walks alone; a scout finding re-opens the walk on the same terms as any lens pass. Freeze and manifest emission are therefore held pending that report and the CEO's go.
**Closing:** w9 met the bar — instruction 0 / record 0; the scout's seven findings are folded and each was author-verified against live state before folding (the loop indentation at `:355`/`:373`; the 25-plan crash population; the 19/0 corpus split behind the seeded-sentence finding; the three unrestricted tag occurrences; the uncorrected corpus bullet at `LESSONS.md:5224`). Walk 9 restructured nothing. `cycle_check` **BAR_MET** · `plan_lint` **0 FAIL, exit 0** · `fold_check` **CLEAN, 18 signals held**. ⚠️ **Frozen at walk 9, then DEPOSITED on the CEO's go (2026-09-03).** The freeze held while authority stopped before deposit; the CEO then authorised it explicitly. The manifest was emitted at BAR_MET by the emitter and its three detector fields authored; `propagation_check=DIVERGENT:50` is declared verbatim as emitted and **all 50 rows were classified before deposit, not shipped unexamined** — every one is the numeral-in-string class (a pin's value matched against the same numeral in ordinary prose: `P7`=555 against the `:552-555` line range, `P4`=05 a partial-numeral match), which is thread 96's rider and carried in `open_forks`. Zero real restatement divergences. Class derives `shop-infra`, so the deposit HOLDS for a human release by design.

## Cycle Manifest
tier: T1
target: scripts/plan_lint.py
target_class: detector
state_space: qa-step-by-the-gate's-predicate x test-scope-none/other/absent x clause-in-the-STEP/elsewhere-in-the-plan/absent x step-headings-present/absent — every dimension read from SYSTEM artifacts over all 543 `Done/*.md`, never from the author's model: the QA-step axis from `gates._gate_is_qa_step` itself and not a proxy (306 QA steps; a header-only proxy gives 310 and was rejected), the scope axis from the parsed `test_scope` field (13 none, 53 absent, 16 plans whose header does not parse at all), the clause axis from the token counts of the 4 real clause-carriers and the 3 non-carriers, and the step-headings axis from the 25 plans carrying no `## STEP N` heading — the population that turns a mis-scoped insertion into a traceback. Cells enumerated as tests 1-9 in STEP 1 Item 2, including the positive control (test 1), the exit-code invariant (test 4) and the loop-placement discriminator (test 8)
mutants: knowledge/mutants/qa-predeclaration-plan_lint.json
class: shop-infra
reads: scripts/plan_lint.py, gates.py, tools/run_check.py, tools/mutation_check.py, tests/test_plan_lint_detector_checks.py, knowledge/decisions/Done, knowledge/decisions/halted-executable-100022.md, knowledge/qa/evidence/eluvian-wiring-pull-2026-08-26/probes-raw.txt, knowledge/mutants/checker-defects-plan_lint.json, lifecycle.db
writes: scripts/plan_lint.py, tests/test_plan_lint_qa_predeclaration.py, knowledge/mutants/qa-predeclaration-plan_lint.json, knowledge/dev-logs/qa-predeclaration-dev-2026-09-03.md, knowledge/qa/evidence/qa-predeclaration-2026-09-03/pytest_full.txt, knowledge/qa/evidence/qa-predeclaration-2026-09-03/probes-raw.txt, knowledge/qa/evidence/qa-predeclaration-2026-09-03/qa-receipt.md
open_forks: promoting (v) from WARN to FAIL once the declaration is in use and the funnel is re-measured — the warn-first house law, a separate plan; the 53-of-306 no-declaration blind spot and the 16-plan unparseable-header population, both stated and NOT addressed here; negation-aware suppression matching, declined with its failure direction named — sibling of the two `cycle_check` negation mutants that SURVIVED at plan 100022 and are still unfixed; (u)'s QA-step predicate wrong on 74 of 861 steps, declined as its own plan and owed a thread; the refuted remedy still standing at `LESSONS.md:5224`, routed and owed a thread; the `propagation_check` numeral-in-string class — thread 96's rider, and the whole of this plan's DIVERGENT count
walks: 9
yields: 4, 4, 2, 1, 1, 1, 1, 0, 0
validation: cycle_check=BAR_MET, plan_lint=0_FAIL, fold_check=PASS, propagation_check=DIVERGENT:50
...
>
> **Item 1 — re-derive the load-bearing pins (P1, P2, P3, P6, P9, P13) and HALT on mismatch.** ⚠️ **P11 is deliberately NOT on the HALT list** — see its own row: it is reproducible only under a stated exclusion, and its CONCLUSION is robust across every variant, so a numeric mismatch there is not a stop condition. Record measured-vs-expected for each. P6 is the plan's foundation and carries its own positive control; run it with the REAL regex imported from `gates`, never retyped:
>
...
>
> ⚠️ **Then re-run P13 — the GATE, not the commands.** Build deposit-shaped copies of **step 2's** deposits in a scratch tree (never a live path — the incident mandate), then call `gates.check` against a simulated step 2. The `parsed` argument is the agent-receipt dict the daemon would supply; construct it exactly as below, or the simulation tests nothing:
>
> ```
> parsed = {
...
> ```
>
> Record `res["passed"]`, `res["is_qa_step"]` and the failure list. Expected: `passed=True`, `is_qa_step=True`, 0 failures. **Then run the negative control:** strip the summary line from the scratch evidence file and re-run — expected exactly `qa_test_result: no parseable pytest summary — cannot certify clean; pausing`. ⛔ **A control that does not fire means the simulation is inert and its pass proves nothing — HALT.** Also record `gates.check(..., 1, ...)`'s `is_qa_step`, which must be `False` — that is P11 confirmed at the gate itself. *(This item exists because the governing lesson of 2026-09-01 — the one this thread produced — is that executing a plan's COMMANDS is not executing the GATES that judge its steps; two full execution passes missed exactly this class.)*
>
...
> 8. ⛔ **the loop-placement test (cold scout S1-1):** two cases in one — (a) a plan whose QA step is NOT last, so a once-only check would read the wrong step's number; and (b) a plan with **no `## STEP` heading at all**, which must produce no traceback. ⚠️ **The fixture idiom Item 2 tells you to clone emits exactly ONE step** (measured: 1 occurrence in `test_plan_lint_detector_checks.py`), so every other test on this list passes whether or not the check sits inside its loop. This test is the only one that discriminates.
> 9. ⚠️ **the predicate regression test:** a NON-QA step whose body merely mentions `Rule 20` — the exact token (u)'s fallback keys on — in a plan whose QA step is elsewhere → the new check stays silent on that step. This is the P11 defect; without this test a later tidier can swap in the neighbouring block's heuristic and no test objects. (Naming the token here is deliberate: describing it instead would dodge (u) by wording rather than by state, and this step's own WARN is the plan's live specimen of P11.)
>
...
> - ⛔ **OPEN YOUR OWN LOOP — this is the finding that would have shipped a crash.** The P2 anchor sits at **4-space function-body indent**; the neighbouring (u) block's `for hl, sn_str in step_headers:` is also at 4 with its body at 8, so **by the anchor the loop has already exited**. A check written "immediately before" the anchor without its own loop runs ONCE, against (u)'s **leaked** `sn` — and on a plan with no step headers at all `sn` is **unbound**, raising `NameError` and killing every check downstream of it. Measured: **25 of 543** `Done/` plans carry zero `## STEP N` headers. Write your own `for hl, sn_str in step_headers:` / `sn = int(sn_str)` and never read (u)'s `sn`. *(Cold scout S1-1, HIGH, author-verified: indentation confirmed at `:355`/`:373`, population confirmed at 25/543. Eight warm walks, eight tests and seven mutants all missed it — the check is four lines of prose whose correctness turns on WHERE they go.)*
...
>
> **Item 5 — write the mutants manifest** `knowledge/mutants/qa-predeclaration-plan_lint.json`, in the shape of `checker-defects-plan_lint.json` (`target`, then `mutants[]` of `name` / `why` / `anchor` / `replacement` / `expect_fail`). At least eight:
...
> - **swap the gate predicate for the neighbouring block's heuristic** → killed by test 9.
> - ⛔ **hoist the check out of its per-step loop** (leaving it at the anchor's own indent, reading the leaked `sn`) → the once-only behaviour and the unbound-name crash; killed by test 8.
...
>
> **Post-conditions:** all nine new tests pass; the funnel re-measures at 9; `mutation_check` reports 8 killed / 0 survived; the lint on a tripping plan exits 0.
...
> ```
> BPY=/Users/marklehn/Developer/bellows/.venv/bin/python
> mkdir -p knowledge/qa/evidence/qa-predeclaration-2026-09-03
...
> 3. **the corpus census:** re-run the census from **Step 1's Item 6** and paste the list. Expected 9 at authoring; **apply that item's supersede rule — report your number, state the delta, and HALT only on a delta that changes the CLASS of the result.**
...
> 5. **the predicate holds:** run the lint on **this very plan** and show the new WARN does NOT fire on its step 1 (its scope is targeted), while the sibling check's WARN does — the P11 instance this plan declares in MUST-PRESERVE. ⚠️ **Resolve this plan's own file by its id, never by a hardcoded name:** `find knowledge/decisions -name '*executable-<id>*.md'` — the claimed file is renamed through `in-progress-` and `verdict-pending-` as it runs, so the name you were dispatched under is not the name on disk. *(576's own walk register records this exact trap at its w3-1; cold scout S1-7, origin-carried.)*
>
> **Item 3 — re-run `tools/mutation_check.py`** on the committed code and paste the kill map. 8 killed / 0 survived.
>
...
>
> ⚠️ **On the QA gate:** **this plan has a real test scope.** Item 1 produces the pytest summary the gate parses, named in the deposits above; no override clause applies here, and none should be copied from this step. ⛔ **Phrased deliberately to carry NONE of the three suppression tokens** — an earlier revision denied needing an override in a sentence that used two of them, and is described here rather than quoted, because reproducing it would reintroduce the token this fold exists to remove. In a doc-only clone of this step that sentence would suppress the very warning it denies needing: a negating sentence is indistinguishable from a real clause under blind spot 2. Measured: 19 QA steps in `Done/` carry that label and **0** lack a real clause, so (v) has zero false suppressions in the shipped corpus — and this step, cloned, would have been the first. *(Cold scout S1-5, MED, author-verified.)*
>
> **Deposits:**
> - `knowledge/qa/evidence/qa-predeclaration-2026-09-03/pytest_full.txt`
> - `knowledge/qa/evidence/qa-predeclaration-2026-09-03/probes-raw.txt`
> - `knowledge/qa/evidence/qa-predeclaration-2026-09-03/qa-receipt.md`
>
> **Post-conditions:** suite green at the derived count; the check fires on 100013; the corpus census matches Step 1 Item 6's supersede rule (9 at authoring, class unchanged); silent on all four clause-carriers; exit code unchanged; kill map 8/8.
