...
**Tier computed, not judged (§1):** **T-1 fires** — six files in one subsystem (`bellows.py`, `depositor.py`, `status.py`, `tools/gate_watcher.py`, `tools/reconcile_plan.py`, tests). **T-2 fires** — the daemon will write a NEW value into production `lifecycle.db` plan rows. **T-8 fires** — not a structure-for-structure clone (100005 is the clone origin BY KIND, not by shape). T-3 does not fire (authored and dispatched on the mini; the shop pulls the same code and restarts on its own schedule — stated in the History-facing note below). T-5 no (every edit is additive; the schema is unchanged — `awaiting_verdict` is ALREADY in the CHECK list, `lifecycle.py:46`). T-6 no (`gates.py` untouched; no doctrine). Highest demand → **T1: full five-lens walk, no panel required; the scout is at the Planner's call and was NOT convened (a T1 code plan whose every claim is measured below).**
...
- ⚠️⚠️ **Every new `mark_plan_state` call is wrapped in `try/except Exception` with a `logger.warning`** — the G4 form at `bellows.py:1023`. A lifecycle write must never turn a pause into a crash.
...
> except Exception:
>     logger.warning(f"lifecycle: failed to write awaiting_verdict for plan {plan_id}")
> ```
...
> **B5 (`bellows.py`)** — anchor (count 1): `lifecycle.record_verdict_request(_lc_plan_id, step_number)` — after it: the same four lines with `_lc_plan_id` in place of `plan_id` (this is inside `_recheck_refuse`, a staticmethod: `logger` is the module logger, in scope).
...
>
> **A4 — post-conditions (counts read, never exit codes):** P3 `mark_plan_state(` == 12; `"awaiting_verdict")` in `bellows.py` == 5; D1/S1/R1 tokens each == 1 post; `lifecycle.py` sha == P1's (untouched); `git diff --stat` names exactly the six source files + the test files, nothing else.
>
...
> - **Item 1 — the edits landed (source probes, raw → `probes-raw.txt`):** P3 == 12; `"awaiting_verdict")` in `bellows.py` == 5; D1, S1, R1 post-tokens == 1 each; `_push_pause` defined once and called once in `tools/gate_watcher.py`; `lifecycle.py` sha == P1's; `git show <DEV commit> --numstat --format=` lists exactly the Scope files.
