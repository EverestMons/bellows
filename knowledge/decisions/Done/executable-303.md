...
>
> ⚠️⚠️ **Task A0-PRE — THE WORK MAY ALREADY BE DONE AND COMMITTED. CHECK THIS FIRST.** A prior dispatch of this plan (**id 302, halted for a Step 2 authoring defect — Step 1 was correct**) committed the three checks as **`cc5f0e8`**. ⚠️ **If `git -C /Users/marklehn/Developer/GitHub/bellows log --oneline -- scripts/plan_lint.py` shows that commit, Step 1 is ALREADY APPLIED:** verify the three checks are present, each a bare `print(...)` touching neither `results` nor `all_passed`; re-run the targeted suite; confirm `plan_lint` exits 0 on a real plan. ⚠️⚠️ **Then do NOT reapply the three checks — that would duplicate them.** **Record the verification in the dev log as evidence, not as new work.** ⚠️ **But Step 1 is NOT complete: one remaining task below.**
>
> ⚠️⚠️ **Task A0-FIX — CHECK (i) HAS A DEMONSTRATED FALSE POSITIVE. TIGHTEN IT BEFORE STEP 2 MEASURES ANYTHING.** Run against THIS plan, `(i)` warns *"plan id `277` in questions region but absent from halt-routing"* — **and this plan is an executable with no halt routing at all.** Three defects in one firing:
> - **It applies to plan classes that have no halt routing.** The concept is diagnostic-specific; an executable has none by design. ⚠️ **Gate `(i)` on the plan having a questions region, and skip silently — with a comment saying why — when it does not.**
> - **Prose matches the directive regex.** The line it matched is a **table row describing the defect class**, not a routing directive.
> - **First match wins** (`break`), so a description early in the file **shadows a real routing line later.**
>
> ⚠️ **Fix the first defect at minimum — it is the one that would fire on every executable in the corpus.** **Report the other two in the dev log with a recommendation; do not silently broaden the check to swallow them.** **Add a regression test: this plan's own text must produce NO `(i)` warning.**
>
> **Task A0 — pre-edit cleanliness + warn-first precondition (277).** *(Only if A0-PRE finds the work absent.)* `git -C /Users/marklehn/Developer/GitHub/bellows status --porcelain -- scripts/plan_lint.py tests/test_plan_lint.py` must be empty. **If DIRTY — resume disambiguation (Rule 56):** grep the dirty files for THIS plan's own edits (the three new check comments, the new test names). All present and attributable → `git restore` both files and reapply from scratch (**NEVER hand-patch a partial apply**). Any unattributable hunk → **HALT, do NOT restore.**
...
5. **Emit the QA Receipt with the canonical Rule 20 self-check block**, a verification row per numbered item above with its raw evidence.
   - `required_evidence_files`: `[targeted-tests.txt, full-suite.txt, corpus-sweep.txt]`
   - ⚠️ **Deposit all three evidence files BEFORE running the block** — it `sys.exit(1)`s if any is missing or empty.
...
> **Scope:**
> - `knowledge/qa/three-checks-qa-report-2026-08-06.md`
...
- **C1** — ⚠️⚠️ **an executable is diffed against the newest same-class plan BEFORE drafting, not after.** This draft left T-8 open, drafted anyway, and the walk-1 diff against `277` found **eight** pieces of dropped machinery: the bootstrap prompt, the CEO-context and how-to-run sections, uppercase step headings, the A0 cleanliness-and-warn-first precondition, verbatim edit anchors, a per-step scope block, the protect-existing-tests task, and the no-heredoc / no-push / STOP conventions. (walk 1)
- ⚠️⚠️ **C1's own first draft QUOTED the scope-block token verbatim while listing it as dropped machinery — and the gate matched that prose as a phantom block inside step 2's extracted text, failing `(d)`.** **The Cycle Log header says gate-matching strings are DESCRIBED here, never quoted, and this entry violated it while recording a different violation.** (deposit, caught by the gate)
