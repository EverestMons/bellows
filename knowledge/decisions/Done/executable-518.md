...
## Build-time corrections (panel seats 1–3, author-verified — these govern where they and the design differ; ⚠️ LATER corrections govern EARLIER ones where they differ: C-9 supersedes C-1's source-role wording, C-15 supersedes C-2/C-10's tail mechanism, C-16 supersedes C-11's recording-move scope — A1 carries the final forms)

- **C-1 — the REQUEST FILE is MANDATORY for any continue (unifies S1-1/S1-9).** Teardown failures NEVER reach gate_events — `record_gate_events` (:1080/:1210) runs BEFORE the teardown try appends its failure (:1110-1116/:1237-1243/:1268-1276; measured: 0 teardown rows in 6438) — and re-run steps legitimately have NO gate_events rows at all (`steps UNIQUE(plan_id, step_number)` makes their recording a silent no-op). The request file is therefore the ONLY carrier for both classes. Corrected rule: **absent request file → refuse as unverifiable, for EVERY plan class** (id-native included — a deleted request file must never read as clean even when gate_events pass rows exist); gate_events' role is fixed by C-9 (the override-annotation layer); a request file PRESENT but carrying no parseable `Gate Result JSON` → refuse as unverifiable likewise.
...
- **C-17 — the census crosses files, and test_bellows.py joins the write set (S3-5, executed — ship-blocker; count finalized at SIXTEEN by S4-3).** tests/test_bellows.py carries EIGHT more casualties (lines ~498/834/877/968/1029/1075/1875/3348 — JSON-less or request-less continues expecting advance); it was outside Scope and Deposits, making Q1's zero-failure gate unreachable by a compliant executor. test_bellows.py is added to A1/A2/A4, Deposits, Scope; the census is SIXTEEN (six C-6/C-12 lines plus the disjoint C-19 pair in test_consume_verdicts, eight in test_bellows); the seat's proto run with all fourteen applied reached 1320 passed / 5 scratch-environment-only failures.
...
- **C-8 — divergences declared (S1-8).** The rename target `processed-rejected-verdict-<slug>-step-<N>.md`, and the override CLI's explicit `<step>` argument are deliberate plan-over-design corrections under this block's governing clause (the row-4 predicate divergence retired with the row — C-11).
...
| X6 | the UPDATE predicate — row 4's FORMER subject, now a fixed landmark (C-11) | lifecycle.py:577-581 | `UPDATE verdicts SET … WHERE plan_id = ? AND step_number = ? AND outcome IS NULL` — UNCHANGED by this plan; the record-once choreography makes overwrite rights unnecessary |
...
| X8 | consumption test surface | **20** tests in tests/test_consume_verdicts.py | must pass unchanged or with the SIXTEEN updates: test_consume_verdicts.py eight (six census + the C-19 pair) and test_bellows.py eight — the design D-6's "2" is thrice-superseded |
...
**Walks:** walk 0 pinned; **walks 1–3 complete** — warm walks 1–2 (one fold, then dry), **the FULL COLD PANEL at the freeze: scout 9 → discovery 9 → execution 7 → capstone 6 = 31 findings (8 HIGH), non-decaying, every one author-verified before folding, execution-seat fixes run with output shown**; walk 3 = the post-panel sweep walk, all lenses dry, exit-gated (residue greps 0).
**Direction verdict (after walk 1): PROCEED — re-tested after the panel and stands:** the panel rebuilt the mechanism's internals (C-1…C-21: precedence inversion, record-once choreography, tail nesting, the sixteen-test census) but validated the shape at every seat.
...
**Cold panel: CONVENED AT THE FREEZE (full form, four seats) — COMPLETE.** The fold set is C-1…C-21 in this plan's corrections block; the capstone's NOT-READY was discharged by its folds + walk 3.
...
**Closing:** **walk 3 met the bar.** Warm series 1 → 0; panel 9 → 9 → 7 → 6. ⚠️ The cycle is CLOSED; the deposit travels the lane WITH the receipt ritual (tool against the DRAFT bytes before staging) → expected HOLD `class:shop-infra` → the CEO's `--release-class-hold` act → claim.
...
**A1 — implement the gap table AS CORRECTED (the C-block governing):**
...
**Q2 — change-set vs the design's gap table:** the bound rows present at their sites as corrected (rows 1-3, 5, 6 plus the C-10/C-17 additions; row 4 verifiably ABSENT per C-11 — the lifecycle.py predicate unchanged) as bound by A1; the fixed landmarks byte-identical — `git diff` scoped to the three retirement call lines, the gate_auto sites, the precondition retry, and the stop branch is EMPTY; `is_runnable_plan`/`is_claimable` untouched; no file outside the five declared.
